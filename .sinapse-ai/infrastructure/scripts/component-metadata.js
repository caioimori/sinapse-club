/**
 * Component Metadata Manager for SINAPSE
...
class ComponentMetadata {
  constructor(options = {}) {
    ...
    this.metadataPath = path.join(this.rootPath, 'sinapse-ai', 'metadata');

    // Initialize memory adapter if available
    this.memoryClient = MemoryAdapter ? new MemoryAdapter({
      persistencePath: path.join(this.rootPath, 'sinapse-memory-layer-mvp', 'data'),
