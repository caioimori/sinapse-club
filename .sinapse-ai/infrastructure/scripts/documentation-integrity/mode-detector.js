 ...
 *
 * Detects installation mode for SINAPSE projects with three-mode support:
...
/**
 * Installation modes supported by SINAPSE
 * @enum {string}
 */
...
const LegacyProjectType = {
  EXISTING_SINAPSE: 'EXISTING_SINAPSE',
  GREENFIELD: 'GREENFIELD',
  BROWNFIELD: 'BROWNFIELD',
  UNKNOWN: 'UNKNOWN',
};
...
const ModeDescriptions = {
  [InstallationMode.FRAMEWORK_DEV]: {
    ...
    hint: 'Developing sinapse-ai itself - uses framework standards, skips project setup',
    description: 'For SINAPSE contributors working on the framework',
  },
  [InstallationMode.GREENFIELD]: {
    label: '🆕 New Project (Greenfield)',
    hint: 'Start a fresh project with SINAPSE - generates project docs, config, and infrastructure',
    description: 'Empty directory setup with full scaffolding',
  },
  [InstallationMode.BROWNFIELD]: {
    label: '📂 Existing Project (Brownfield)',
    hint: 'Add SINAPSE to existing project - analyzes current structure and adapts',
 ...
 * 1. FRAMEWORK_DEV - .sinapse-ai/ exists AND is sinapse-ai repo
...
function detectInstallationMode(targetDir = process.cwd()) {
  ...
  // Priority 1: Check for SINAPSE framework development
  if (markers.hasSinapseCore && markers.isSinapseCoreRepo) {
    return {
      mode: InstallationMode.FRAMEWORK_DEV,
      legacyType: LegacyProjectType.EXISTING_SINAPSE,
      confidence: 100,
      reason: 'Detected sinapse-ai repository with .sinapse-ai directory',
      markers,
    };
  }

  // Priority 2: Check for existing SINAPSE installation (non-framework)
  if (markers.hasSinapseCore && !markers.isSinapseCoreRepo) {
    return {
      mode: InstallationMode.BROWNFIELD,
      legacyType: LegacyProjectType.EXISTING_SINAPSE,
      confidence: 95,
      reason: 'SINAPSE already installed in user project - treating as brownfield update',
      markers,
    };
  }
...
function collectMarkers(targetDir) {
  ...
  return {
    // SINAPSE markers
    hasSinapseCore: fs.existsSync(path.join(targetDir, '.sinapse-ai')),
    isSinapseCoreRepo: isSinapseCoreRepository(targetDir),
...
/**
 * Checks if the target directory is the sinapse-ai repository itself
 *
 * @param {string} targetDir - Directory to check
 * @returns {boolean} True if this is the sinapse-ai repository
 */
function isSinapseCoreRepository(targetDir) {
  ...
  try {
    ...
    // Primary check: explicit sinapse-ai package names
    if (packageJson.name === '@sinapse/core' || packageJson.name === 'sinapse-ai') {
      return true;
    }

    // Secondary check: workspaces pattern + sinapse-specific marker file
    // This prevents false positives for generic monorepos
    const hasSinapseMarker = fs.existsSync(path.join(targetDir, '.sinapse-ai', 'infrastructure'));
    const hasWorkspaces =
      Array.isArray(packageJson.workspaces) && packageJson.workspaces.includes('packages/*');

    return hasWorkspaces && hasSinapseMarker;
  } catch (error) {
    // Log error for debugging but don't throw - return false for safety
    if (process.env.SINAPSE_DEBUG) {
      console.warn(`[mode-detector] Error checking sinapse-ai repository: ${error.message}`);
    }
    return false;
  }
}
 ...
 * for EXISTING_SINAPSE legacy type (both use EXISTING_SINAPSE but with different modes).
 * For accurate mode detection, use the mode property from detectInstallationMode().
 *
 * @param {string} legacyType - Legacy project type (EXISTING_SINAPSE, GREENFIELD, etc.)
 * @param {Object} [context] - Optional context for disambiguation
 * @param {boolean} [context.isSinapseCoreRepo] - True if this is the sinapse-ai repository
 * @returns {string} New installation mode
 */
function mapLegacyTypeToMode(legacyType, context = {}) {
  // EXISTING_SINAPSE is ambiguous: it can be FRAMEWORK_DEV (sinapse-ai repo)
  // or BROWNFIELD (user project with SINAPSE installed)
  if (legacyType === LegacyProjectType.EXISTING_SINAPSE) {
    // Use context to disambiguate if provided
    if (context.isSinapseCoreRepo === true) {
      return InstallationMode.FRAMEWORK_DEV;
    }
    if (context.isSinapseCoreRepo === false) {
...
function validateModeSelection(selectedMode, detected) {
  ...
  if (selectedMode !== detected.mode) {
    ...
    if (selectedMode === InstallationMode.FRAMEWORK_DEV && !detected.markers.isSinapseCoreRepo) {
      result.warnings.push(
        'Selected framework-dev but this does not appear to be the sinapse-ai repository.',
      );
    }
