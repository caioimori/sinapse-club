...
const GitignoreTemplates = {
  SINAPSE_BASE: 'gitignore-sinapse-base.tmpl',
...
function getTemplatesForStacks(techStacks) {
  const templates = [GitignoreTemplates.SINAPSE_BASE];
...
function generateGitignore(markers, options = {}) {
  ...
  sections.push('# Generated by SINAPSE Documentation Integrity System');
...
/**
 * Merges SINAPSE ignores with existing .gitignore
 *
...
function mergeGitignore(existingContent, _options = {}) {
  // Check if SINAPSE section already exists
  if (existingContent.includes('SINAPSE Integration Section')) {
    console.log('SINAPSE section already exists in .gitignore, skipping merge');
...
/**
 * Checks if a .gitignore file has SINAPSE integration
 *
 * @param {string} targetDir - Target directory
 * @returns {boolean} True if SINAPSE section exists
 */
function hasSinapseIntegration(targetDir) {
  const gitignorePath = path.join(targetDir, '.gitignore');
  ...
  return content.includes('SINAPSE Integration Section') || content.includes('.sinapse-ai/');
}
...
function parseGitignore(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('SINAPSE Integration Section')) {
      if (sinapseStart === -1) {
        sinapseStart = i;
      }
    }

    if (line.includes('End of SINAPSE Integration Section')) {
      sinapseEnd = i;
    }

    if (line.startsWith('#') && !line.includes('SINAPSE')) {
  ...
  if (sinapseStart !== -1) {
    sections.sinapseSection = {
      start: sinapseStart,
      end: sinapseEnd !== -1 ? sinapseEnd : lines.length - 1,
    };
  }
