 ...
 * for display in agent greetings across all 11 SINAPSE agents.
 *
...
class ProjectStatusLoader {
  ...
  loadConfig() {
    try {
      const configPath = path.join(this.rootPath, '.sinapse-ai', 'core-config.yaml');
  ...
  _resolveCacheFilePath() {
    try {
      ...
      if (normalizedGitDir !== normalizedCommonDir) {
        ...
        return path.join(this.rootPath, '.sinapse', `project-status-${worktreeHash}.yaml`);
      }
    } catch (error) {
      // Not a git repo or git not available - use default path
    }

    return path.join(this.rootPath, '.sinapse', 'project-status.yaml');
  }
 ...
 * @property {string} path - Relative path to worktree (.sinapse/worktrees/{story-id})
