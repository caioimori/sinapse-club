...
Automatically synchronizes SINAPSE agent definitions to IDE command files.

## Overview

IDE Sync keeps agent definitions in `.sinapse-ai/development/agents/` synchronized with IDE-specific command files in:

- `.claude/commands/SINAPSE/agents/` (Claude Code)
- `.codex/agents/` (Codex CLI support files)
- `.gemini/rules/SINAPSE/agents/` (Gemini CLI)
...
npm run sync:ide
# or
node .sinapse-ai/infrastructure/scripts/ide-sync/index.js sync
```
...
# or
node .sinapse-ai/infrastructure/scripts/ide-sync/index.js validate
```
...
# or
node .sinapse-ai/infrastructure/scripts/ide-sync/index.js validate --strict
```
...
Configure in `.sinapse-ai/core-config.yaml`:
...
```
.sinapse-ai/infrastructure/scripts/ide-sync/
