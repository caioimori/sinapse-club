...
const CORE_CONFIG_FILE = '.sinapse-ai/core-config.yaml';
 ...
 * - .sinapse-ai/core/** gets expanded one level deep
...
function expandProtectedPaths(protectedPaths, exceptions, projectRoot) {
  ...
  for (const globPath of protectedPaths) {
    if (globPath === '.sinapse-ai/core/**') {
      const { dirs, files } = expandOneLevel('.sinapse-ai/core', projectRoot);
