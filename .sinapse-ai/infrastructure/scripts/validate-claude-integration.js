...
function validateClaudeIntegration(options = {}) {
  ...
  const agentsDir = options.agentsDir || path.join(projectRoot, '.claude', 'commands', 'SINAPSE', 'agents');
  const hooksDir = options.hooksDir || path.join(projectRoot, '.claude', 'hooks');
  const sourceAgentsDir =
    options.sourceAgentsDir || path.join(projectRoot, '.sinapse-ai', 'development', 'agents');
