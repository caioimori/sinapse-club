 ...
 *
 * Features:
 * - AC1: Located in `.sinapse-ai/infrastructure/scripts/`
 ...
 * - AC7: Integrates with `.sinapse/dashboard/status.json`
 *
...
class PlanTracker {
  ...
  _findPlanPath() {
    const searchPaths = [
      ...
      // .sinapse plans location
      path.join(this.rootPath, '.sinapse/plans', this.storyId, CONFIG.implementationFile),
      // Epic-based location (e.g., sinapse-ai-ade/story-4.6)
   ...
   *
   * Updates `.sinapse/dashboard/status.json` for dashboard integration.
   * Also updates legacy `.sinapse/status.json` for backwards compatibility.
   *
...
async function main() {
  ...
  if (args.length < 1 || args.includes('--help') || args.includes('-h')) {
    console.log(`
\u{1F4CA} Plan Progress Tracker - SINAPSE Execution Engine (Story 4.6)
