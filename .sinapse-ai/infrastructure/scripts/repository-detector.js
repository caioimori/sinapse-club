 ...
 * @property {string} frameworkLocation - Path to SINAPSE framework files
...
function detectRepositoryContext() {
  ...
  // Detect if we're in sinapse-ai repo itself
  const isFrameworkRepo =
    packageJson.name === '@sinapse/fullstack' ||
    packageJson.name === 'sinapse-ai' ||
    remoteUrl.includes('sinapse-ai');

  // Load installation config if exists
  let installConfig = null;
  const configPath = path.join(cwd, '.sinapse-installation-config.yaml');
  ...
  return {
    ...
    projectRoot: cwd,
    frameworkLocation: isFrameworkRepo ? cwd : path.join(cwd, 'node_modules/@sinapse/fullstack'),
