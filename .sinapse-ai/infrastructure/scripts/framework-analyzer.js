...
/**
 * Framework structure analyzer for SINAPSE
 * Discovers and catalogs all framework components
 */
class FrameworkAnalyzer {
  constructor(options = {}) {
    this.rootPath = options.rootPath || process.cwd();
    this.sinapseCoreDir = path.join(this.rootPath, 'sinapse-ai');
  ...
  async discoverAgents() {
    const agents = [];
    const agentsDir = path.join(this.sinapseCoreDir, 'agents');
  ...
  async discoverTasks() {
    const tasks = [];
    const tasksDir = path.join(this.sinapseCoreDir, 'tasks');
  ...
  async discoverWorkflows() {
    const workflows = [];
    const workflowsDir = path.join(this.sinapseCoreDir, 'workflows');
  ...
  async discoverUtils() {
    const utils = [];
    const utilsDir = path.join(this.sinapseCoreDir, 'utils');
  ...
  async discoverTemplates() {
    const templates = [];
    const templatesDir = path.join(this.sinapseCoreDir, 'templates');
