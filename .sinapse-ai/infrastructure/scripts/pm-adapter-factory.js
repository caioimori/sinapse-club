 ...
 * Automatically selects the correct adapter based on .sinapse-pm-config.yaml.
 *
 * Falls back to LocalAdapter if no PM tool is configured, ensuring
 * SINAPSE works 100% standalone without external dependencies.
 *
 ...
 *
 * Reads .sinapse-pm-config.yaml from project root and instantiates
 * the appropriate adapter. Caches the adapter for subsequent calls.
 *
 * Falls back to LocalAdapter if:
 * - No .sinapse-pm-config.yaml file exists
...
function getPMAdapter() {
  ...
  const configPath = path.join(process.cwd(), '.sinapse-pm-config.yaml');

  // If no PM config file, use local-only adapter
  if (!fs.existsSync(configPath)) {
    console.log('ℹ️  No PM tool configured - using local-only mode');
    console.log('   To configure a PM tool, run: sinapse init');
 ...
 *
 * Reads .sinapse-pm-config.yaml and returns the tool type.
