...
const CONFIG = {
  ...
  // Workflow definition
  workflowPath: '.sinapse-ai/development/workflows/qa-loop.yaml',
  ...
  persistenceIndexPath: '.sinapse/qa-loops-index.json', // Track all active loops
};
...
class QALoopOrchestrator {
  ...
  _loadMaxIterations() {
    ...
    const configPaths = [
      path.join(this.rootPath, '.sinapse/config.yaml'),
      path.join(this.rootPath, '.sinapse/config.yml'),
      path.join(this.rootPath, 'sinapse.config.js'),
    ];
...
function printHelp() {
  console.log(`
📊 QA Loop Orchestrator - SINAPSE QA Evolution (Story 6.5)
