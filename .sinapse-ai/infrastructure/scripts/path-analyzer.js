...
const REFERENCE_PATTERNS = [
  // Markdown code refs to .sinapse-ai
  { pattern: /`\.sinapse-ai\/[^`]+`/g, type: 'code-ref' },
  // Task references
...
const ASSET_DIRS = {
  task: '.sinapse-ai/development/tasks',
  template: '.sinapse-ai/product/templates',
  checklist: '.sinapse-ai/product/checklists',
  script: '.sinapse-ai/infrastructure/scripts',
  data: '.sinapse-ai/development/data',
  schema: '.sinapse-ai/schemas',
  agent: '.sinapse-ai/development/agents',
};
...
async function findFilesToAnalyze(rootPath) {
  const files = [];
  const dirsToScan = [
    { dir: '.sinapse-ai/development/agents', type: 'agent' },
    { dir: '.sinapse-ai/development/tasks', type: 'task' },
    { dir: '.sinapse-ai/product/templates', type: 'template' },
    { dir: '.sinapse-ai/product/checklists', type: 'checklist' },
  ];
...
function resolveReference(ref, rootPath) {
  ...
  if (ref.reference.startsWith('.sinapse-ai/')) {
...
function formatConsoleOutput(report, verbose = false) {
  ...
  lines.push('');
  lines.push('═══════════════════════════════════════════════════════════');
  lines.push('  SINAPSE Path Analysis Report');
...
Options:
  ...
  while (rootPath !== '/') {
    try {
      await fs.access(path.join(rootPath, '.sinapse-ai'));
      break;
    } catch {
  ...
  if (rootPath === '/') {
    console.error('Error: Could not find .sinapse-ai directory. Run from project root.');
    process.exit(1);
  }
