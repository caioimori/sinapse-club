...
// Layer: L2 (.sinapse-ai/infrastructure/scripts/)
// Purpose:
...
//
// Usage:
//   node .sinapse-ai/infrastructure/scripts/generate-optimization-report.js [options]
//
// Options:
...
const ANALYTICS_DIR = path.resolve(ROOT, '.sinapse', 'analytics');
...
const TOOL_REGISTRY_FILE = path.resolve(ROOT, '.sinapse-ai', 'data', 'tool-registry.yaml');
...
function compareBaseline(baseline, usageData, registry) {
  ...
  for (const [wfName, wfData] of Object.entries(baselineWorkflows)) {
    ...
    const baselineOverheadPct = baseline.comparison?.sinapseActual?.overheadPercentOfTypicalSession?.[wfName] || 0;
