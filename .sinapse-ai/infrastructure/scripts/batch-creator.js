/**
 * Batch Component Creator for SINAPSE
...
class BatchCreator {
    ...
    for (let i = 1; i <= answers.stepCount; i++) {
      ...
      components.push({
        ...
        config: {
          ...
          agentName: 'sinapse-developer', // Default to meta-agent
        },
      });
    }
  ...
  async initTransactionLog() {
    const logDir = path.join(this.rootPath, 'sinapse-ai', 'logs', 'transactions');
