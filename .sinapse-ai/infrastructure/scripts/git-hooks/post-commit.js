...
/**
 * Git Post-Commit Hook - SINAPSE ProjectStatusLoader Cache Invalidation
 *
 ...
 *     node .sinapse-ai/infrastructure/scripts/git-hooks/post-commit.js
 */
 ...
 * looking for .sinapse-ai directory.
 *
...
function findProjectRoot() {
  ...
  // Walk up looking for .sinapse-ai
  for (let i = 0; i < 10; i++) {
    if (fs.existsSync(path.join(dir, '.sinapse-ai'))) {
      return dir;
    }
...
/**
 * Clear all project-status cache files in the .sinapse directory.
...
function clearProjectStatusCache() {
  ...
  if (!fs.existsSync(sinapseDir)) {
    return; // No .sinapse directory - nothing to clear
  }

  try {
    const files = fs.readdirSync(sinapseDir);

    for (const file of files) {
      // Match project-status.yaml and project-status-{hash}.yaml
      if (file.startsWith('project-status') && file.endsWith('.yaml')) {
        const filePath = path.join(sinapseDir, file);
        try {
