/**
 * Component Generator for SINAPSE
...
class ComponentGenerator {
  constructor(options = {}) {
    ...
    this.templatesPath = path.join(this.rootPath, 'sinapse-ai', 'templates');
    this.outputPaths = {
      agent: path.join(this.rootPath, 'sinapse-ai', 'agents'),
      task: path.join(this.rootPath, 'sinapse-ai', 'tasks'),
      workflow: path.join(this.rootPath, 'sinapse-ai', 'workflows'),
    };
  ...
  processAnswersToVariables(componentType, answers) {
    ...
    switch (componentType) {
      ...
      case 'task':
        ...
        // Notes
        variables.EACH_NOTES = ['Generated using SINAPSE template system'];
        break;
      ...
      case 'workflow':
        ...
        // Metadata
        variables.AUTHOR = process.env.USER || 'SINAPSE';
  ...
  async createAgentCommandTasks(variables, options) {
    ...
    for (const command of variables.COMMANDS) {
      const taskId = this.commandToTaskId(command.COMMAND_NAME);
      const taskPath = path.join(this.rootPath, 'sinapse-ai', 'tasks', `${taskId}.md`);
