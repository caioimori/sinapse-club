...
Base layer of the SINAPSE modular architecture. Contains tools, integrations, scripts, and PM adapters.
...
- `ComponentGenerator` - SINAPSE component generation
- `BatchCreator` - Batch operations

### Validation
- `SinapseValidator` - SINAPSE component validation
...
const {
  ...
  TemplateEngine,
  resolveTool
} = require('.sinapse-ai/infrastructure');

// Or import directly from scripts
const GitWrapper = require('.sinapse-ai/infrastructure/scripts/git-wrapper');
```

## Tool Resolution

```javascript
const { resolveTool } = require('.sinapse-ai/infrastructure');
...
```javascript
const { getPMAdapter, isPMToolConfigured } = require('.sinapse-ai/infrastructure');
