...
class JiraAdapter extends PMAdapter {
   ...
   * @param {string} config.project_key - Project key (e.g., "SINAPSE")
   */
  constructor(config) {
    super(config);
  ...
  async pullStory(storyId) {
    try {
      ...
      // Map Jira status to SINAPSE status
      const statusMapping = {
        'To Do': 'Draft',
  ...
  async updateStatus(storyId, status) {
    try {
      ...
      // Map SINAPSE status to Jira transition
   ...
   * Get Jira issue
   * @private
   * @param {string} issueKey - Issue key (e.g., "SINAPSE-123")
