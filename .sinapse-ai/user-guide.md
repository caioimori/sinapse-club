...
O SINAPSE é um framework de desenvolvimento full stack orientado por agentes de IA que implementa uma metodologia ágil única. Este guia explica como usar o SINAPSE de forma eficaz através de suas duas fases principais: **Planejamento** e **Desenvolvimento**.
...
### Instalando SINAPSE-FullStack

O SINAPSE-FullStack oferece instalação simplificada através de um instalador interativo completo.
...
npx @sinapse/fullstack install
```

**O que acontece durante a instalação**:

```
🚀 SINAPSE-FullStack Installation Wizard
...
✓ Installing .sinapse-ai/ framework...
✓ Installing squads...
...
Se você está contribuindo para o SINAPSE-FullStack em si:
...
```
your-project/
├── .sinapse-ai/                    # ✅ Framework core instalado
...
Se você já tem SINAPSE instalado e quer fazer upgrade:

```bash
# Upgrade automático (RC.9+)
npx sinapse-ai@latest install --force-upgrade

# OU interativo (todos os RCs)
npx sinapse-ai@latest install
# → Menu aparece:
#   1. Keep current version (v1.0.0-rc.8)
#   2. Upgrade SINAPSE core (v1.0.0-rc.8 → v1.0.0-rc.10)
...
npm view sinapse-ai@latest version
# Output: 1.0.0-rc.10

# Ver opções de instalação e comandos disponíveis
npx sinapse-ai@latest --help
...
# Ajuda do instalador
npx sinapse-ai@latest install --help
```
...
O futuro comando `sinapse init` permitirá escolher explicitamente entre dois modos:

#### Framework Development Mode
- Para desenvolvedores contribuindo ao SINAPSE-FullStack
- `.sinapse-ai/` é código fonte (commitado)
- Mudanças afetam o framework
- Quality gates testam integridade do framework

#### Project Development Mode
- Para desenvolvedores usando SINAPSE em seus projetos
- `.sinapse-ai/` é dependência (gitignored)
...
# Cria .sinapse-installation-config.yaml
...
npx sinapse-ai@1.0.0-rc.10 install --force-upgrade
```
...
O SINAPSE usa uma abordagem de duas fases que separa planejamento estratégico de implementação tática:
...
O agente **qa** do SINAPSE inclui capacidades avançadas de arquitetura de testes:
...
2. **Recursos especiais**:
   - Reconhecimento automático de comandos SINAPSE
   - Integração com workflows e tasks
   - Rastreamento automático de checkboxes

## Sistema de Preferências Técnicas

O SINAPSE permite configurar preferências técnicas que influenciam decisões dos agentes:
...
O SINAPSE-FullStack impõe integridade de repositório através de um agente centralizado de DevOps.
...
O SINAPSE-FullStack oferece três modos de desenvolvimento para acomodar diferentes níveis de habilidade e restrições de tempo.
...
Ao integrar SINAPSE em projetos existentes:
...
O **sinapse-orchestrator** gerencia fluxos de trabalho:
...
O **sinapse-developer** é o meta-agente para o próprio SINAPSE:
...
[sinapse-developer guiará você na criação de um novo agente]
```
...
Use o **sinapse-developer** para criar agentes customizados:
...
Adicione templates próprios em `sinapse-ai/templates/`:
...
- [ ] IDE configurado com regras SINAPSE
...
- [Arquitetura do SINAPSE](../docs/architecture.md)
