...
SINAPSE Presets are pre-configured bundles that provide ready-to-use configurations for specific use cases, project types, or team structures. Presets simplify the initial setup process and ensure consistent configuration across projects.
...
| ------------ | ----------------------- | ------------------ | ----------------- |
| `minimal`    | Bare minimum SINAPSE setup | dev                | Quick prototyping |
...
\`\`\`bash
sinapse install --preset my-preset
\`\`\`
```
...
sinapse install --preset standard

# Install with multiple presets (layered)
sinapse install --preset standard --preset frontend

# Install custom preset from directory
sinapse install --preset ./presets/my-preset
```
...
sinapse presets list

# List with details
sinapse presets list --verbose
```
...
sinapse presets show standard

# Compare two presets
sinapse presets diff minimal standard
```
...
sinapse presets export --name my-project-preset --output ./presets/

# Export with description
sinapse presets export --name my-preset --description "Project template" --output ./presets/
```
...
```bash
sinapse install --preset standard --preset frontend --preset small-team
```
...
2. **Review before install** - Use `sinapse presets show` to understand what you're getting
3. **Export before modifying** - Save your config as a preset before major changes
4. **Keep presets updated** - Check for preset updates when upgrading SINAPSE
