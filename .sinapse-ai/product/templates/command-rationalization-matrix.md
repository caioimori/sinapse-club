...
grep -r "\*command-name" docs/ .sinapse-ai/ | wc -l

# For each agent's commands:
for cmd in $(grep -E "^  - [a-z-]+" .sinapse-ai/agents/AGENT.md | cut -d':' -f1 | sed 's/  - //'); do
  count=$(grep -r "\*$cmd" docs/ .sinapse-ai/ 2>/dev/null | wc -l)
  echo "$cmd: $count"
done
```
...
```yaml
- create {type} {name}: Create SINAPSE component
