#!/bin/bash
#
# SINAPSE Framework Documentation Migration Script
#
# Purpose: Migrate framework docs from sinapse-fullstack to sinapse-ai (REPO 1)
...
#   --target-repo    Path to sinapse-ai repository (default: ../sinapse-ai)
...
echo ""
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  SINAPSE Framework Documentation Migration Script             ║${NC}"
...
if [ ! -d "$SOURCE_DOCS" ]; then
  ...
  echo "  Please run this script from the sinapse-fullstack root directory"
  exit 1
fi
...
if [ ! -d "$TARGET_REPO" ]; then
  ...
  echo "  Please clone sinapse-ai first or specify correct path with --target-repo"
  exit 1
fi
...
**Source:** sinapse-fullstack/docs/framework/
**Target:** sinapse-ai/docs/framework/
**Story:** 6.1.2.6 - Framework Configuration System

## Migration Status

✅ Framework documentation successfully migrated from sinapse-fullstack to sinapse-ai (REPO 1)

## Files Migrated

- coding-standards.md - Official SINAPSE coding standards
...
- sinapse-fullstack maintains copies in `docs/framework/` (will be removed Q3 2026)
- sinapse-fullstack maintains legacy copies in `docs/architecture/` (will be removed Q3 2026)
...
```bash
# Restore from sinapse-fullstack
cd sinapse-fullstack
...
if [ "$DRY_RUN" = false ] && [ -d "$TARGET_REPO/.git" ]; then
  ...
  echo "    git commit -m \"docs: migrate framework documentation from sinapse-fullstack\""
  echo ""
  echo "    Story: 6.1.2.6 - Framework Configuration System"
  echo "    Migrated: coding-standards.md, tech-stack.md, source-tree.md"
  echo "    Source: sinapse-fullstack/docs/framework/"
  echo ""
...
echo "    4. Commit changes to sinapse-ai repository"
echo "    5. Update sinapse-fullstack deprecation warnings (Q3 2026)"
echo ""
