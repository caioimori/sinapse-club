...
$allTasks = Get-ChildItem .sinapse-ai/tasks/*.md | Where-Object { $_.Name -notlike '*v1-backup*' } | ForEach-Object { $_.Name }
...
foreach ($task in $allTasks) {
  $result = node .sinapse-ai/scripts/validate-task-v2.js ".sinapse-ai/tasks/$task" 2>&1
...
foreach ($task in $nonCompliantTasks) {
  ...
  node .sinapse-ai/scripts/migrate-task-to-v2.js ".sinapse-ai/tasks/$task" 2>&1 | Out-Null
