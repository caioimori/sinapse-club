# Diagnóstico de problema com npx sinapse-ai
# Execute este script no PC com problema

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "   Diagnóstico NPX sinapse-ai" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[4] Baixando sinapse-ai manualmente..." -ForegroundColor Yellow
$tempDir = "$env:TEMP\sinapse-test-$(Get-Random)"
New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
Set-Location $tempDir

Write-Host "    Diretório: $tempDir"
npm pack sinapse-ai@2.2.0 2>&1 | Out-Null

if (Test-Path "sinapse-ai-2.2.0.tgz") {
    Write-Host "    Download: OK" -ForegroundColor Green

    # Extrair
    tar -xzf sinapse-ai-2.2.0.tgz 2>&1 | Out-Null

    if (Test-Path "package/bin/sinapse.js") {
        Write-Host "    Extração: OK" -ForegroundColor Green

        # Tentar executar
        Write-Host ""
        Write-Host "[5] Executando bin/sinapse.js --version:" -ForegroundColor Yellow
        try {
            $result = node package/bin/sinapse.js --version 2>&1
        ...
        Write-Host "    Executando: node package/bin/sinapse.js --help"
        node package/bin/sinapse.js --help 2>&1

    } else {
        Write-Host "    Extração: FALHOU - bin/sinapse.js não encontrado" -ForegroundColor Red
