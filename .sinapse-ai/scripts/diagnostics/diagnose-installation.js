...
const pkgInfo = exec('npm view sinapse-ai version 2>&1');
if (pkgInfo && !pkgInfo.includes('E404') && !pkgInfo.includes('error')) {
  console.log(`  ✓ sinapse-ai v${pkgInfo} is available on npm`);
} else {
  console.log('  ⚠️  Cannot verify sinapse-ai package availability');
...
console.log('  Testing: npx sinapse-ai@latest --version');
const npxTest = exec('npx sinapse-ai@latest --version 2>&1');
...
const summaryFile = path.join(os.tmpdir(), 'sinapse-diagnostic-result.json');
