...
SINAPSE-Method is a framework that combines AI agents with Agile development methodologies. The v4 system introduces a modular architecture with improved dependency management, bundle optimization, and support for both web and IDE environments.
...
SINAPSE transforms you into a "Vibe CEO" - directing a team of specialized AI agents through structured workflows. Here's how:
...
**Note for VS Code Users**: SINAPSE-Method assumes when you mention "VS Code" that you're using it with an AI-powered extension like GitHub Copilot. Standard VS Code without AI capabilities cannot run SINAPSE agents.

**Verify Installation**:

- `.sinapse-ai/` folder created with all agents
- IDE-specific integration files created
- All agent commands/rules/modes available

**Remember**: At its core, SINAPSE-Method is about mastering and harnessing prompt engineering. Any IDE with AI agent support can use SINAPSE - the framework provides the structured prompts and workflows that make AI development effective
...
**New in V4**: The `sinapse-ai/core-config.yaml` file is a critical innovation that enables SINAPSE to work seamlessly with any project structure, providing maximum flexibility and backwards compatibility.

### What is core-config.yaml?

This configuration file acts as a map for SINAPSE agents, telling them exactly where to find your project documents and how they're structured. It enables:
...
3. **Custom Workflows**: Configure SINAPSE to match your team's process
...
The SINAPSE-Method is built around a modular architecture centered on the `sinapse-ai` directory, which serves as the brain of the entire system. This design enables the framework to operate effectively in both IDE environments (like Cursor, VS Code) and web-based AI interfaces (like ChatGPT, Gemini).

### Key Architectural Components

#### 1. Agents (`sinapse-ai/agents/`)
...
#### 2. Agent Teams (`sinapse-ai/agent-teams/`)

- **Purpose**: Define collections of agents bundled together for specific purposes
- **Examples**: `team-all.yaml` (comprehensive bundle), `team-fullstack.yaml` (full-stack development)
- **Usage**: Creates pre-packaged contexts for web UI environments

#### 3. Workflows (`sinapse-ai/workflows/`)
...
- **Templates** (`sinapse-ai/templates/`): Markdown templates for PRDs, architecture specs, user stories
- **Tasks** (`sinapse-ai/tasks/`): Instructions for specific repeatable actions like "shard-doc" or "create-next-story"
- **Checklists** (`sinapse-ai/checklists/`): Quality assurance checklists for validation and review
- **Data** (`sinapse-ai/data/`): Core knowledge base and technical preferences
...
SINAPSE employs a sophisticated template system with three key components:

1. **Template Format** (`utils/sinapse-doc-template.md`): Defines markup language for variable substitution and AI processing directives from yaml templates
...
This architecture enables seamless operation across environments while maintaining the rich, interconnected agent ecosystem that makes SINAPSE powerful.
...
SINAPSE integrates with ClickUp for project management and story tracking. When creating stories using the `create-next-story` task, agents must follow a specific workflow to correctly interact with the ClickUp MCP server.
...
- Complete workflow: `sinapse-ai/tools/mcp/clickup.yaml` (story_creation_workflow section)
- Task instructions: `sinapse-ai/tasks/create-next-story.md` (sections 5.1 and 5.3)
- Validators: `sinapse-ai/tools/mcp/clickup.yaml` (executable_knowledge section)
...
## Contributing to SINAPSE-Method
...
Squads extend SINAPSE-Method beyond traditional software development into ANY domain. They provide specialized agent teams, templates, and workflows while keeping the core framework lean and focused on development.
