 ...
 *
 * @module sinapse-ai/doctor/fix-handler
 * @story INS-4.1
 */
...
const FIX_MAP = {
  ...
  'agent-memory': {
    ...
    async apply(result, { projectRoot }) {
      const agentsDir = path.join(projectRoot, '.sinapse-ai', 'development', 'agents');
      let created = 0;
  ...
  'claude-md': {
    ...
    async apply() {
      return 'CLAUDE.md regeneration requires npx sinapse-ai install --force';
    },
  },
  ...
  'settings-json': {
    ...
    async apply(result, { frameworkRoot }) {
      ...
      return 'settings.json regeneration requires npx sinapse-ai install --force';
    },
  },
};
