...
SINAPSE now automatically translates Portuguese documentation into English filenames when sharding documents. This ensures consistency across all projects and compatibility with hardcoded English filenames in configuration files.
...
**See full dictionary:** `.sinapse-ai/tasks/shard-doc.md` (Section 3)
...
**Configuration:** `.sinapse-ai/core-config.yaml`
...
```bash
# Check for issues
node .sinapse-ai/scripts/validate-filenames.js

# Auto-fix issues (renames files)
node .sinapse-ai/scripts/validate-filenames.js --fix
```
...
Edit `.sinapse-ai/tasks/shard-doc.md`, Section 3:
...
**Solution:**
1. Check you're using the updated `.sinapse-ai/tasks/shard-doc.md`
2. Run validator to confirm: `node .sinapse-ai/scripts/validate-filenames.js`
...
**Manual fix:**
Edit `.sinapse-ai/core-config.yaml` to add more fallbacks.
...
```javascript
// .sinapse-ai/scripts/validate-filenames.js
...
A: Edit `.sinapse-ai/tasks/shard-doc.md`, Section 3, and add to the YAML dictionary.
...
2. Run the validator: `node .sinapse-ai/scripts/validate-filenames.js`
3. Review `.sinapse-ai/tasks/shard-doc.md` for translation rules
