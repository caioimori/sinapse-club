# SINAPSE Meta-Agent Troubleshooting Guide

## Overview

This guide helps diagnose and resolve common issues when using the SINAPSE meta-agent for component creation and management.
...
**Symptoms:**
```
❌ Error: Agent 'data-analyst' already exists at /sinapse-ai/agents/data-analyst.md
```
...
3. If overwriting is intended:
   ...
   rm sinapse-ai/agents/data-analyst.md
   *create-agent
   ```
...
1. Create session directory:
   ```bash
   mkdir -p sinapse-ai/.sessions
   ```

2. Check permissions:
   ```bash
   chmod 755 sinapse-ai/.sessions
   ```
...
2. Check transaction directory:
   ```bash
   ls sinapse-ai/logs/transactions/
   ```
...
1. Manual cleanup:
   ```bash
   # Check failed files
   ls -la sinapse-ai/agents/file1.md
   
   # Remove manually if needed
   rm sinapse-ai/agents/file1.md
   ```
...
3. Restore from backup:
   ```bash
   # Check backups
   ls sinapse-ai/logs/transactions/txn-*/backups/
   ```
...
1. Check failure details:
   ...
   cat sinapse-ai/logs/transactions/latest.json
   ```
...
# Specific modules
DEBUG=sinapse:template,sinapse:elicitation *create-agent
...
```bash
# Application logs
tail -f sinapse-ai/logs/sinapse-developer.log

# Transaction logs
ls -la sinapse-ai/logs/transactions/

# Error logs
grep ERROR sinapse-ai/logs/*.log
```
...
- Template syntax: `sinapse-ai/docs/template-syntax.md`
- Creation guide: `sinapse-ai/docs/component-creation-guide.md`
- API reference: `sinapse-ai/docs/api-reference.md`
...
1. **Backup current state**:
   ```bash
   tar -czf sinapse-backup.tar.gz sinapse-ai/
   ```

2. **Reset to clean state**:
   ```bash
   git checkout -- sinapse-ai/
   npm run setup
   ```
