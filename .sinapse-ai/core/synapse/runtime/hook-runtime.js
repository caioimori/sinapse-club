...
function getStaleSessionTTL(cwd) {
  try {
    ...
    const configPath = path.join(cwd, '.sinapse-ai', 'core-config.yaml');
...
function resolveHookRuntime(input) {
  ...
  try {
    const { loadSession, createSession, cleanStaleSessions } = require(
      path.join(cwd, '.sinapse-ai', 'core', 'synapse', 'session', 'session-manager.js'),
    );
    const { SynapseEngine } = require(
      path.join(cwd, '.sinapse-ai', 'core', 'synapse', 'engine.js'),
    );
