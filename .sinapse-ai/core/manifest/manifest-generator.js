 ...
 * Scans the .sinapse-ai directory structure to build comprehensive manifests.
 *
...
class ManifestGenerator {
  constructor(options = {}) {
    ...
    this.sinapseCoreDir = path.join(this.basePath, '.sinapse-ai');
    this.manifestDir = path.join(this.sinapseCoreDir, 'manifests');
    this.version = '2.1.0';
  }
  ...
  async generateAgentsManifest() {
    const agentsDir = path.join(this.sinapseCoreDir, 'development', 'agents');
        ...
        try {
          ...
          if (parsed && parsed.agent) {
            ...
            agents.push({
              ...
              status: 'active',
              file_path: `.sinapse-ai/development/agents/${file}`,
  ...
  async generateWorkersManifest() {
    const registryPath = path.join(this.sinapseCoreDir, 'core', 'registry', 'service-registry.json');
  ...
  async generateTasksManifest() {
    const tasksDir = path.join(this.sinapseCoreDir, 'development', 'tasks');
      ...
      for (const file of mdFiles) {
        try {
          ...
          tasks.push({
            ...
            file_path: `.sinapse-ai/development/tasks/${file}`,
            status: 'active',
          });
