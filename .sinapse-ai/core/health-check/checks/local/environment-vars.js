 ...
 *
 * @module sinapse-ai/health-check/checks/local/environment-vars
 * @version 1.0.0
...
/**
 * SINAPSE-specific environment variables
 */
const SINAPSE_VARS = ['SINAPSE_DEBUG', 'SINAPSE_LOG_LEVEL'];
...
class EnvironmentVarsCheck extends BaseCheck {
  ...
  async execute(_context) {
    ...
    // Check SINAPSE-specific vars (info only)
    for (const varName of SINAPSE_VARS) {
      if (process.env[varName]) {
        sinapseVars.push({ name: varName, value: this.maskValue(process.env[varName]) });
      }
    }
