 ...
 *
 * @module sinapse-ai/health-check/checks/project/workflow-dependencies
 * @version 1.0.0
...
class WorkflowDependenciesCheck extends BaseCheck {
  constructor() {
    super({
      ...
      cacheable: true,
      healingTier: 0,
      tags: ['sinapse', 'workflows', 'dependencies'],
    });
  }
  ...
  async execute(context) {
    ...
    const workflowPaths = [
      path.join(projectRoot, '.sinapse-ai', 'development', 'workflows'),
      path.join(projectRoot, '.sinapse-ai', 'infrastructure', 'workflows'),
    ];
    ...
    const taskPaths = [
      path.join(projectRoot, '.sinapse-ai', 'development', 'tasks'),
      path.join(projectRoot, '.sinapse-ai', 'infrastructure', 'tasks'),
    ];
