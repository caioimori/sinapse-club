 ...
 */
const CONFIG_FILES = {
  framework: '.sinapse-ai/framework-config.yaml',
  project: '.sinapse-ai/project-config.yaml',
  pro: 'pro/pro-config.yaml',
  local: '.sinapse-ai/local-config.yaml',
  legacy: '.sinapse-ai/core-config.yaml',
  user: path.join(os.homedir(), '.sinapse', 'user-config.yaml'),
};
...
function loadLayeredConfig(projectRoot, options = {}) {
  ...
  if (options.appDir) {
    const appConfigPath = path.join(options.appDir, 'sinapse-app.config.yaml');
  ...
  // L5: User (optional — global user preferences, cross-project, ~/.sinapse/user-config.yaml)
...
function loadLegacyConfig(projectRoot) {
  ...
  const suppressDeprecation = process.env.SINAPSE_SUPPRESS_DEPRECATION === 'true'
    || process.env.SINAPSE_SUPPRESS_DEPRECATION === '1';

  if (!suppressDeprecation) {
    warnings.push(
      '[DEPRECATION] Monolithic core-config.yaml detected. '
      + 'Run `sinapse config migrate` to split into layered config files. '
      + 'Monolithic format will be removed in v4.0.0. '
      + 'Set SINAPSE_SUPPRESS_DEPRECATION=true to silence this warning.',
    );
  }
...
function getConfigAtLevel(projectRoot, level, options = {}) {
  ...
  switch (level) {
    ...
    case 'app': case '3': case 'L3':
      ...
      relativePath = path.join(options.appDir, 'sinapse-app.config.yaml');
      break;
...
/**
 * Ensure the ~/.sinapse/ directory exists with secure permissions.
 *
...
/**
 * Set a value in the user config file (~/.sinapse/user-config.yaml).
