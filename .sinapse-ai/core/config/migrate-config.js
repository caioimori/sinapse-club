 ...
 *   - .sinapse-ai/project-config.yaml (L2) — team-shared project settings
 *   - ~/.sinapse/user-config.yaml (L5) — cross-project user preferences
 *
 * Safety:
...
/**
 * Fields that belong in L5 User config (~/.sinapse/user-config.yaml).
...
/**
 * Fields that belong in L2 Project config (.sinapse-ai/project-config.yaml).
...
function isLegacyMode(projectRoot) {
  const legacyPath = path.join(projectRoot, '.sinapse-ai', 'core-config.yaml');
  const frameworkPath = path.join(projectRoot, '.sinapse-ai', 'framework-config.yaml');
...
function createBackup(projectRoot) {
  const legacyPath = path.join(projectRoot, '.sinapse-ai', 'core-config.yaml');
  const backupPath = path.join(projectRoot, '.sinapse-ai', 'core-config.backup.yaml');
...
/**
 * Ensure the ~/.sinapse/ directory exists.
 *
 * @returns {string} Path to ~/.sinapse/ directory
 */
function ensureUserConfigDir() {
  const dir = path.join(os.homedir(), '.sinapse');
...
/**
 * Write user config to ~/.sinapse/user-config.yaml.
...
function writeUserConfig(userFields) {
  ensureUserConfigDir();
  const userConfigPath = path.join(os.homedir(), '.sinapse', 'user-config.yaml');
...
/**
 * Write project config to .sinapse-ai/project-config.yaml.
...
function writeProjectConfig(projectRoot, projectFields) {
  const projectConfigPath = path.join(projectRoot, '.sinapse-ai', 'project-config.yaml');
