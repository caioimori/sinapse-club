 ...
 */
const DEFAULT_CONFIG_PATH = '.sinapse-ai/core/quality-gates/quality-gate-config.yaml';
...
class QualityGateManager {
   ...
   */
  async getStatus() {
    const statusPath = this.config.status?.location || '.sinapse/qa-status.json';
  ...
  async saveReport(storyId = 'unknown') {
    const reportDir = this.config.reports?.location || '.sinapse/qa-reports';
