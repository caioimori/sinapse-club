...
/**
 * Write HTML graph to .sinapse/graph.html and open in default browser.
...
function handleHtmlOutput(graphData, options = {}) {
  const outputDir = path.resolve(process.cwd(), '.sinapse');
 ...
 * Writes to .sinapse/graph.dot, .sinapse/graph.mmd, or .sinapse/graph.html.
 * @param {Object} args - Parsed CLI args
...
async function handleWatch(args) {
  ...
  const registryPath = path.resolve(process.cwd(), '.sinapse-ai/data/entity-registry.yaml');
...
function handleHelp() {
  const usage = `
Usage: sinapse graph [command] [options]
...
Examples:
  ...
  sinapse graph --deps --format=json          Output as JSON
  sinapse graph --deps --format=html          Interactive HTML graph (opens browser)
  sinapse graph --deps --watch                Live DOT file for VS Code preview
  sinapse graph --deps --watch --format=html  Live HTML with auto-refresh
  sinapse graph --deps --watch --format=mermaid  Live Mermaid file
  sinapse graph --deps --watch --interval=10  Refresh every 10 seconds
  sinapse graph --stats                       Show entity stats and cache metrics
`.trim();
...
async function handleSummary(args) {
  ...
  sections.push('SINAPSE Graph Dashboard');
