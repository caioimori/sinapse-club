 ...
 *
 * Features:
 * - AC1: Updates `.sinapse/dashboard/status.json`
...
class DashboardIntegration extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    // Paths
    this.dashboardDir = path.join(this.projectRoot, '.sinapse', 'dashboard');
    this.statusPath = path.join(this.dashboardDir, 'status.json');
    this.logsDir = path.join(this.projectRoot, '.sinapse', 'logs');
