 ...
 *
 * Features:
 * - AC1: Located in `.sinapse-ai/core/orchestration/`
...
class MasterOrchestrator extends EventEmitter {
  ...
  async loadState(storyId = null) {
    ...
    const targetPath = targetStoryId
      ? path.join(this.projectRoot, '.sinapse', 'master-orchestrator', `${targetStoryId}.json`)
      : this.statePath;
  ...
  async findLatestValidState() {
    const stateDir = path.join(this.projectRoot, '.sinapse', 'master-orchestrator');
