...
class BobOrchestrator {
    ...
    try {
      ...
      if (!lockAcquired) {
        return {
          ...
          projectState: null,
          action: 'lock_failed',
          error: 'Another Bob orchestration is already running. Wait or check .sinapse/locks/',
        };
      }
  ...
  async _handleNoConfig(_context) {
    ...
    return {
      action: 'onboarding',
      data: {
        message: 'Projeto sem configuração SINAPSE detectado. Iniciando onboarding...',
        nextStep: 'run_sinapse_init',
      },
    };
  }
