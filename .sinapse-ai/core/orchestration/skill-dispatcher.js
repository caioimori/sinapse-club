/**
 * Skill Dispatcher - Dispatches to SINAPSE Skills instead of generic Task calls
 *
 ...
 *
 * Responsibilities:
 * - Map agent IDs to SINAPSE Skill names
 ...
 * @property {string} skill - Full skill name (e.g., 'SINAPSE:agents:architect')
...
/**
 * Dispatches workflow phases to SINAPSE Skills
 */
/**
...
class SkillDispatcher {
  ...
  constructor(options = {}) {
     ...
     * These correspond to files in .claude/commands/SINAPSE/agents/
     */
    this.skillMapping = {
      // Core development agents
      architect: 'SINAPSE:agents:architect',
      'data-engineer': 'SINAPSE:agents:data-engineer',
      dev: 'SINAPSE:agents:dev',
      qa: 'SINAPSE:agents:qa',

      // Product agents
      pm: 'SINAPSE:agents:pm',
      po: 'SINAPSE:agents:po',
      sm: 'SINAPSE:agents:sm',
      analyst: 'SINAPSE:agents:analyst',

      // Specialized agents
      'ux-design-expert': 'SINAPSE:agents:ux-design-expert',
      'ux-expert': 'SINAPSE:agents:ux-design-expert', // Alias
      devops: 'SINAPSE:agents:devops',
   ...
   * @returns {string} Full skill name (e.g., 'SINAPSE:agents:architect')
   */
  getSkillName(agentId) {
    return this.skillMapping[agentId] || `SINAPSE:agents:${agentId}`;
  }
  ...
  getAgentPersona(agentId) {
    return (
      this.agentPersonas[agentId] || { name: agentId, title: 'SINAPSE Agent' }
    );
  }
  ...
  isValidAgent(agentId) {
    return agentId in this.skillMapping || agentId.startsWith('SINAPSE:');
  }
}
