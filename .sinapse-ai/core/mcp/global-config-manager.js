 ...
 *
 * Manages the global MCP configuration at ~/.sinapse/mcp/
...
/**
 * Check if global SINAPSE directory exists
 * @returns {boolean}
 */
function globalDirExists() {
  return fs.existsSync(getGlobalSinapseDir());
}
...
function addServer(serverName, serverConfig = null) {
  ...
  if (!config) {
    return { success: false, error: 'Global config not found. Run "sinapse mcp setup" first.' };
  }
