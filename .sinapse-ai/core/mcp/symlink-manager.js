 ...
 * @returns {string} Path to .sinapse-ai/tools/mcp
 */
function getProjectMcpPath(projectRoot = process.cwd()) {
  return path.join(projectRoot, '.sinapse-ai', 'tools', 'mcp');
}
...
function createLink(projectRoot = process.cwd(), options = {}) {
  ...
  if (!fs.existsSync(globalPath)) {
    return {
      success: false,
      error: 'Global MCP directory does not exist. Run "sinapse mcp setup" first.',
      linkPath,
      globalPath,
    };
  }
