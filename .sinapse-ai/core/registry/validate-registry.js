...
async function validateFields(registryPath) {
  ...
  for (const worker of registry.workers) {
    ...
    // Check path format
    if (worker.path && !worker.path.startsWith('.sinapse-ai/')) {
      issues.push({ id: worker.id, issue: 'Invalid path prefix (must start with .sinapse-ai/)' });
    }
  }
...
async function main() {
  ...
  const registryPath = path.join(baseDir, '.sinapse-ai/core/registry/service-registry.json');
  const schemaPath = path.join(baseDir, '.sinapse-ai/core/registry/registry-schema.json');
