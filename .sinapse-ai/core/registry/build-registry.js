 ...
 *
 * Scans SINAPSE modules and builds the service registry JSON file.
...
const SCAN_SOURCES = [
  {
    pattern: '.sinapse-ai/development/tasks/**/*.md',
    category: 'task',
  ...
  {
    pattern: '.sinapse-ai/product/templates/**/*.md',
    category: 'template',
    taskFormat: 'TEMPLATE',
    subcategoryExtractor: (filePath) => {
  ...
  {
    pattern: '.sinapse-ai/infrastructure/scripts/**/*.js',
    category: 'script',
    taskFormat: 'SCRIPT',
    subcategoryExtractor: (filePath) => {
  ...
  {
    pattern: '.sinapse-ai/product/checklists/**/*.md',
    category: 'checklist',
    taskFormat: 'CHECKLIST',
    subcategoryExtractor: () => 'quality',
  },
  {
    pattern: '.sinapse-ai/development/workflows/**/*.yaml',
    category: 'workflow',
    taskFormat: 'WORKFLOW',
    subcategoryExtractor: (filePath) => {
  ...
  {
    pattern: '.sinapse-ai/core/data/**/*.md',
    category: 'data',
    taskFormat: 'TASK-FORMAT-V1',
    subcategoryExtractor: () => 'knowledge',
  },
  {
    pattern: '.sinapse-ai/core/data/**/*.yaml',
    category: 'data',
    taskFormat: 'TASK-FORMAT-V1',
...
async function main() {
  ...
  const outputPath = process.argv[3] || path.join(baseDir, '.sinapse-ai/core/registry/service-registry.json');
