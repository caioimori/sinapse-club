...
The Entity Registry is the central data store for the Incremental Development System (IDS). It tracks all SINAPSE framework artifacts — tasks, templates, scripts, modules, agents, checklists, and data files — with metadata, relationships, adaptability scores, and checksums.

## Schema

The registry is stored at `.sinapse-ai/data/entity-registry.yaml` as a single YAML file.
...
categories:
  - id: "tasks"
    ...
    basePath: ".sinapse-ai/development/tasks"
```
...
```js
const { RegistryLoader } = require('./.sinapse-ai/core/ids/registry-loader');
...
```bash
node .sinapse-ai/development/scripts/populate-entity-registry.js
```
