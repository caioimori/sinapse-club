 ...
 *
 * Subcommands:
 *   sinapse config show [--level <n>] [--app <name>] [--debug]
 *   sinapse config diff --levels <a>,<b>
 *   sinapse config migrate [--dry-run] [--force]
 *   sinapse config validate [--level <n>]
...
// Resolve core config modules (relative from .sinapse-ai/cli/commands/config/)
...
function migrateAction(options) {
  ...
  try {
    ...
    if (originalStr === resolvedStr) {
      ...
      console.log('This may be expected due to key normalization. Run `sinapse config diff` to inspect.');
    }
...
function splitL1(config) {
  ...
  l1.metadata = { name: 'SINAPSE', framework_version: '3.12.0' };
...
function initLocalAction() {
  const root = getProjectRoot();
  const templatePath = path.join(root, '.sinapse-ai', 'local-config.yaml.template');
  const targetPath = path.join(root, '.sinapse-ai', 'local-config.yaml');

  if (!fs.existsSync(templatePath)) {
    console.error('Template not found: .sinapse-ai/local-config.yaml.template');
    process.exit(1);
  }
  ...
  console.log('Created: .sinapse-ai/local-config.yaml (from template)');
...
/**
 * Create the `sinapse config` command with all subcommands.
 * @returns {Command}
 */
