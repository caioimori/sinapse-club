...
Data Storage:
  Metrics are stored in .sinapse/data/quality-metrics.json
...
Examples:
  $ sinapse metrics record --layer 1 --passed --duration 3200
  $ sinapse metrics record --layer 2 --passed --findings 3 --coderabbit
  $ sinapse metrics show
  $ sinapse metrics show --layer 2
  $ sinapse metrics show --format json
  $ sinapse metrics seed --days 30
  $ sinapse metrics cleanup
  $ sinapse metrics cleanup --dry-run
`);
