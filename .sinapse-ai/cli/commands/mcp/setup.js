 ...
 * MCP Setup Command
 *
 * Creates the global ~/.sinapse/mcp/ directory structure.
 *
...
function createSetupCommand() {
  ...
  setup
    .description('Create global MCP configuration structure at ~/.sinapse/')
...
async function executeSetup(options) {
  ...
  console.log(`  ${getGlobalSinapseDir()}/`);
  console.log('  ├── mcp/');
  ...
  console.log('\nNext steps:');
  console.log('  1. Run "sinapse mcp link" in your project to use global config');
  console.log('  2. Run "sinapse mcp add <server>" to add more servers');
  console.log('  3. Run "sinapse mcp status" to check configuration');
