...
async function executeAdd(server, options) {
  ...
  if (options.listTemplates) {
    ...
    console.log('Usage: sinapse mcp add <server-name>');
    console.log('Example: sinapse mcp add context7');
    return;
  }
  ...
  if (!globalConfigExists()) {
    ...
    console.log('Run "sinapse mcp setup" first to create global configuration.');
    process.exit(1);
  }
  ...
  if (!server) {
    ...
    console.log('  sinapse mcp add myserver --type sse --url https://example.com/mcp');
    console.log('  sinapse mcp add myserver --command npx --args "-y,@scope/mcp-server"');
    process.exit(1);
  }
  ...
  if (result.success) {
    ...
    if (result.error.includes('No template available')) {
      ...
      console.log(`  sinapse mcp add ${server} --type sse --url <url>`);
      console.log(`  sinapse mcp add ${server} --command npx --args "-y,@scope/server"`);
    }
