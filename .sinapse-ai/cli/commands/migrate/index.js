...
function printHeader(fromVersion, toVersion) {
  console.log('');
  console.log(`🔄 SINAPSE Migration v${fromVersion} → v${toVersion}`);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
}
...
async function runMigration(options) {
  ...
  if (!versionInfo.isV2) {
    ...
    console.error(`❌ ${versionInfo.error || 'Cannot detect SINAPSE version'}`);
    process.exit(1);
  }
  ...
  try {
    ...
    if (!migrationResult.success) {
      ...
      console.log('  Run: sinapse migrate --rollback');
      process.exit(1);
    }
  ...
  try {
    ...
    const importResult = await updateAllImports(sinapseCoreDir, plan, { verbose, onProgress });
  ...
  const backupDir = `.sinapse-backup-${new Date().toISOString().split('T')[0]}`;
...
async function runRollbackCommand(options) {
  ...
  console.log('');
  console.log('🔙 SINAPSE Migration Rollback');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
...
async function showBackupStatus(_options) {
  ...
  console.log('');
  console.log('📦 SINAPSE Migration Backups');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
...
function createMigrateCommand() {
  ...
  migrate
    .description('Migrate SINAPSE from v2.0 to v4.0.4 modular structure')
...
Migration Flow:
  Phase 1: Backup
    - Creates .sinapse-backup-{date}/ with all files
...
Examples:
  $ sinapse migrate                          # Interactive migration
  $ sinapse migrate --dry-run                # Preview changes
  $ sinapse migrate --verbose                # Detailed output
  $ sinapse migrate -y --skip-tests          # Non-interactive, skip tests
  $ sinapse migrate --rollback               # Restore from backup
  $ sinapse migrate --status                 # Show backups

Rollback:
  If migration fails or you want to revert:
  $ sinapse migrate --rollback

  Manual rollback:
  $ cp -r .sinapse-backup-{date}/* .sinapse-ai/
`);
