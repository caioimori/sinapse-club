...
/**
 * Create backup of .sinapse-ai directory
...
async function createBackup(projectRoot, options = {}) {
  ...
  if (!fs.existsSync(sinapseCoreDir)) {
    throw new Error('No .sinapse-ai directory found. Is this an SINAPSE v2.0 project?');
  }
  ...
  const files = await getAllFiles(sinapseCoreDir);
  const manifest = {
    version: '2.0',
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const relativePath = path.relative(sinapseCoreDir, srcFile);
    const destFile = path.join(backupDir, '.sinapse-ai', relativePath);
...
async function verifyBackup(backupDir) {
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new Error('Backup manifest not found. Is this a valid SINAPSE backup?');
  }
  ...
  for (const file of manifest.files) {
    const filePath = file.isConfig
      ...
      : path.join(backupDir, '.sinapse-ai', file.relativePath);
...
async function findLatestBackup(projectRoot) {
  ...
  const backups = entries
    .filter(entry => entry.isDirectory() && entry.name.startsWith('.sinapse-backup-'))
    .map(entry => ({
      name: entry.name,
      path: path.join(projectRoot, entry.name),
      date: entry.name.replace('.sinapse-backup-', ''),
    }))
...
async function listBackups(projectRoot) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith('.sinapse-backup-')) {
