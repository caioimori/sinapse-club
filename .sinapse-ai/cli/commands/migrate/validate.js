 ...
 */
async function validateStructure(sinapseCoreDir) {
  const result = {
    valid: true,
    modules: {},
    errors: [],
  ...
  const rootEntries = await fs.promises.readdir(sinapseCoreDir, { withFileTypes: true });
...
async function runFullValidation(projectRoot, options = {}) {
  ...
  result.structure = await validateStructure(sinapseCoreDir);
  ...
  result.imports = await verifyImports(sinapseCoreDir);
...
function generateSummary(migrationResult, validationResult, options = {}) {
  ...
  lines.push('  3. Report issues: github.com/sinapse/issues');
  lines.push('');
  lines.push('To rollback: sinapse migrate --rollback');
