 ...
 */
async function removeV21Structure(sinapseCoreDir, options = {}) {
...
async function restoreFromBackup(backup, projectRoot, options = {}) {
  ...
  const backupSinapseCoreDir = path.join(backup.path, '.sinapse-ai');

  // Restore .sinapse-ai files
  ...
  for (const file of backup.manifest.files) {
    try {
      const sourcePath = file.isConfig
        ...
        : path.join(backupSinapseCoreDir, file.relativePath);

      const targetPath = file.isConfig
        ? path.join(projectRoot, file.relativePath)
        : path.join(sinapseCoreDir, file.relativePath);
...
async function executeRollback(projectRoot, options = {}) {
  ...
  const sinapseCoreDir = path.join(projectRoot, '.sinapse-ai');
  result.removal = await removeV21Structure(sinapseCoreDir, { onProgress });
  ...
  for (const dir of expectedDirs) {
    if (fs.existsSync(path.join(sinapseCoreDir, dir))) {
      validCount++;
    }
  }
...
function formatRollbackSummary(result) {
  ...
  lines.push('');
  lines.push('🔙 SINAPSE Migration Rollback');
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
