...
async function detectV2Structure(projectRoot) {
  ...
  if (!fs.existsSync(sinapseCoreDir)) {
    return {
      ...
      isV21: false,
      version: null,
      error: 'No .sinapse-ai directory found',
    };
  }
  ...
  const hasV21Structure = v21Modules.every(module =>
    fs.existsSync(path.join(sinapseCoreDir, module)),
  );
  ...
  const hasV20Structure = v20Indicators.some(dir =>
    fs.existsSync(path.join(sinapseCoreDir, dir)),
  );
  ...
  return {
    ...
    isV21: false,
    version: null,
    error: 'Unable to detect SINAPSE version structure',
  };
}

/**
 * Categorize file into target module
 * @param {string} relativePath - Relative path from .sinapse-ai
...
async function analyzeMigrationPlan(projectRoot, options = {}) {
  ...
  // Get all files
  const allFiles = await getAllFiles(sinapseCoreDir);
  ...
  for (const filePath of allFiles) {
    const relativePath = path.relative(sinapseCoreDir, filePath);
    ...
    if (module && plan.modules[module]) {
      ...
      fileInfo.targetPath = path.join(sinapseCoreDir, module, relativePath);
  ...
  for (const moduleName of Object.keys(plan.modules)) {
    const targetDir = path.join(sinapseCoreDir, moduleName);
