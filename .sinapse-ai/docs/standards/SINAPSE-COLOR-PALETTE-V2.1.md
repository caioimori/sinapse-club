# SINAPSE Color Palette v4.2

**Version:** 2.1.0  
**Created:** 2025-01-20  
**Status:** Active  
**Scope:** All SINAPSE CLI tools, installer, and visual outputs
...
The SINAPSE color palette is derived from the brand logo's vibrant gradient:
...
:root {
  /* Brand Colors */
  --sinapse-primary: #8B5CF6;
  --sinapse-secondary: #EC4899;
  --sinapse-tertiary: #3B82F6;
  
  /* Functional Colors */
  --sinapse-success: #10B981;
  --sinapse-warning: #F59E0B;
  --sinapse-error: #EF4444;
  --sinapse-info: #06B6D4;
  
  /* Neutral Colors */
  --sinapse-muted: #94A3B8;
  --sinapse-dim: #64748B;
  
  /* Gradient */
  --sinapse-gradient: linear-gradient(135deg, #EC4899 0%, #8B5CF6 50%, #3B82F6 100%);
}
```
...
- Use `colors.brandPrimary` (purple bold) for SINAPSE branding
- Example: Welcome screens, major section headers

```javascript
console.log(colors.brandPrimary('🎉 SINAPSE v4.2 Installer'));
```
...
// Success
console.log(colors.success('\n✓ All done! Your SINAPSE project is ready.\n'));
```
  ...
  "sinapse": {
    "color": {
      "primary": {
        "$type": "color",
        "$value": "#8B5CF6",
        "$description": "SINAPSE primary brand color (ClickUp-inspired purple)"
      },
      "secondary": {
        "$type": "color",
        "$value": "#EC4899",
        "$description": "SINAPSE secondary brand color (magenta accent)"
      },
      "success": {
        "$type": "color",
        "$value": "#10B981",
    ...
    "gradient": {
      "brand": {
        ...
        "$description": "SINAPSE brand gradient (logo-inspired)"
      }
    }
  }
}
...
|---------|------|---------|--------|
| 2.1.0 | 2025-01-20 | Initial SINAPSE Color Palette - Logo-inspired gradient + ClickUp purple | Uma (UX-Design Expert) |

---

## 🔗 Related Documents

- [SINAPSE-LIVRO-DE-OURO-V2.1](./SINAPSE-LIVRO-DE-OURO-V2.1-SUMMARY.md) - Complete SINAPSE standards
