/**
 * SINAPSE Skill Validator
 * Story GEMINI-INT.5 - Skills Cross-CLI Compatibility
 *
 * Validates that SINAPSE agent skills work correctly in both
...
class SkillValidator {
  constructor(config = {}) {
    this.agentsDir = config.agentsDir || path.join(process.cwd(), '.sinapse-ai', 'development', 'agents');
    this.tasksDir = config.tasksDir || path.join(process.cwd(), '.sinapse-ai', 'development', 'tasks');
    this.errors = [];
    this.warnings = [];
  }
  ...
  validateGeminiCompatibility(agentDef, result) {
    // Gemini CLI uses .gemini/rules/SINAPSE/agents/ structure
  ...
  generateReport(results) {
    let report = '# SINAPSE Skills Cross-CLI Compatibility Report\n\n';
