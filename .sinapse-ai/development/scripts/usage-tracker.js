...
/**
 * Usage tracker for SINAPSE-FULLSTACK framework components
 * Tracks component usage patterns for deprecation warnings and impact analysis
 */
class UsageTracker {
  constructor(options = {}) {
    this.rootPath = options.rootPath || process.cwd();
    this.usageDir = path.join(this.rootPath, '.sinapse', 'usage');
  ...
  async getComponentInfo(componentId) {
    ...
    return {
      ...
      type: type,
      name: name,
      file_path: `sinapse-ai/${type}s/${name}.md`
    };
  }

  async getScanPaths(_options) {
    const defaultPaths = [
      path.join(this.rootPath, 'sinapse-ai'),
