 ...
 * Can be overridden via core-config.yaml pipeline.timeout_ms or SINAPSE_PIPELINE_TIMEOUT env var.
 * @type {number}
 */
...
class UnifiedActivationPipeline {
  ...
  async _loadCoreConfig() {
    try {
      const configPath = path.join(this.projectRoot, '.sinapse-ai', 'core-config.yaml');
   ...
   * Priority: SINAPSE_PIPELINE_TIMEOUT env > core-config.yaml pipeline.timeout_ms > default
   * @private
  ...
  _resolvePipelineTimeout(coreConfig) {
    ...
    const envTimeout = process.env.SINAPSE_PIPELINE_TIMEOUT;
    if (envTimeout) {
