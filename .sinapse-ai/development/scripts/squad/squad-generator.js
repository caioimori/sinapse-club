...
/**
 * Default SINAPSE minimum version
 * @constant {string}
 */
const DEFAULT_SINAPSE_MIN_VERSION = '2.1.0';
...
class SquadGeneratorError extends Error {
  ...
  static schemaNotFound(schemaPath) {
    return new SquadGeneratorError(
      ...
      'Ensure SINAPSE is properly installed',
    );
  }
}
...
sinapse:
  minVersion: "${config.sinapseMinVersion || DEFAULT_SINAPSE_MIN_VERSION}"
  type: squad
...
${config.description || 'Custom SINAPSE squad.'}
...
This squad ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'does not inherit'} the core SINAPSE configuration.
...
> This file ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'is independent of'} the core SINAPSE coding standards.
...
- Node.js >= 18.x
- SINAPSE >= ${config.sinapseMinVersion || DEFAULT_SINAPSE_MIN_VERSION}
...
class SquadGenerator {
   ...
   * @param {string} [config.sinapseMinVersion] - Minimum SINAPSE version
  ...
  async generate(config) {
    ...
    const fullConfig = {
      ...
      sinapseMinVersion: config.sinapseMinVersion || DEFAULT_SINAPSE_MIN_VERSION,
