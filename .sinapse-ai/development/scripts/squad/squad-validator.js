      ...
      try {
        ...
        if (!hasYamlFrontmatter && !hasMarkdownHeading) {
          result.warnings.push({
            ...
            file: agentFile,
            message: 'Agent file may not follow SINAPSE agent definition format',
            suggestion:
  ...
  async validateWorkflows(squadPath) {
    ...
    try {
      ...
      result.warnings.push({
        ...
        suggestion: 'Ensure workflow-validator.js exists in .sinapse-ai/development/scripts/',
      });
      return result;
    }

    const coreAgentsPath = path.join(process.cwd(), '.sinapse-ai', 'development', 'agents');
