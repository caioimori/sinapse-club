...
class SquadMigrator {
  ...
  async analyze(squadPath) {
    ...
    try {
      ...
      // Check for missing sinapse.type
      if (!manifest.sinapse?.type) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_SINAPSE_TYPE',
          message: 'Missing required field: sinapse.type',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
      ...
      // Check for missing sinapse.minVersion
      if (!manifest.sinapse?.minVersion) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_MIN_VERSION',
          message: 'Missing required field: sinapse.minVersion',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
          path: 'sinapse.minVersion',
          value: '2.1.0',
        });
      }
