 ...
 *
 * Downloads squads from the sinapse-squads GitHub repository.
...
/**
 * Default registry URL for sinapse-squads
 * @constant {string}
 */
const REGISTRY_URL =
...
/**
 * Squad Downloader class for downloading squads from sinapse-squads repository
 */
class SquadDownloader {
  ...
  /**
   * Fetch registry from sinapse-squads repository
   *
  ...
  async _downloadSquadFiles(squadInfo, targetPath) {
    ...
    if (!Array.isArray(contents)) {
      throw new SquadDownloaderError(
        ...
        'Check if squad exists in sinapse-squads repository',
      );
    }
  ...
  _fetch(url, useApi = false) {
    return new Promise((resolve, reject) => {
      const options = {
        headers: {
          'User-Agent': 'SINAPSE-SquadDownloader/1.0',
        },
      };
