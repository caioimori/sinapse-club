...
To help users contribute to the SINAPSE open-source project (`sinapse-ai`) by automating the PR creation process, ensuring contributions follow project standards, pass quality checks, and have proper formatting before submission.

**Target Repository**: `sinapse-ai` (open-source framework)
...
- **contribution_path**: `string`
  ...
  - **Example**: `"Squads/my-new-pack/"` or `"sinapse-ai/agents/improved-agent.md"`
...
2. **Validate Repository State**
   - Check if `sinapse-ai` repository is set as upstream
...
3. **Detect Contribution Type** (if not provided)
   - Scan modified files:
     - `Squads/*` → "Squad"
     - `sinapse-ai/agents/*` → "agent"
     - `sinapse-ai/tasks/*` → "task"
     - `sinapse-ai/tools/*` → "tool"
...
14. **Generate PR Title & Description**
    ...
    - **Description**: Use PR template:
      ...
      **First-time contributor?** Welcome! 🎉 This PR was created using SINAPSE PR Automation.
      ```
...
- [ ] Fork of sinapse-ai exists
  - **Validation**: `gh repo view {user}/sinapse-ai` succeeds
...
```bash
sinapse activate Otto  # github-devops agent
sinapse pr create \
  --type="Squad" \
...
sinapse pr create \
  --type="agent" \
  --path="sinapse-ai/agents/improved-po.md" \
...
sinapse pr create \
  --type="bug-fix" \
  --path="sinapse-ai/tasks/create-next-story.md" \
