...
- Target workflow must exist (path resolved from target_context):
  - `core` → `.sinapse-ai/development/workflows/`
...
- Resolve workflow path based on target_context:
  - `core` → `.sinapse-ai/development/workflows/{workflow-name}.yaml`
...
- Create timestamped backup in same context:
  - `core` → `.sinapse-ai/development/workflows/.backups/{workflow-name}.yaml.{timestamp}`
