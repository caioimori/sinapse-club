...
```bash
node bin/sinapse-ids.js ids:health
```
...
```bash
node bin/sinapse-ids.js ids:health --fix
```
...
- Log all healing actions to `.sinapse-ai/data/registry-healing-log.jsonl`

### Step 3: JSON Output (Machine-Readable)

```bash
node bin/sinapse-ids.js ids:health --json
node bin/sinapse-ids.js ids:health --fix --json
```
...
```javascript
const { RegistryHealer } = require('.sinapse-ai/core/ids/registry-healer');
