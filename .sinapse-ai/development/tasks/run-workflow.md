...
- campo: workflow_state
  tipo: object
  destino: File system (.sinapse/{instance-id}-state.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] For action=continue/status/skip/abort, an active state file must exist
    ...
    blocker: true
    validação: |
      Check .sinapse/{instance-id}-state.yaml exists with status=active
...
- **Tool:** workflow-state-manager
  ...
  - **Implementation:** AI agent reads/writes `.sinapse/{instance-id}-state.yaml` files directly
...
- For engine mode: `run-workflow-engine.md` task must exist at `.sinapse-ai/development/tasks/run-workflow-engine.md`
- State directory `.sinapse/` must be writable
...
3. **Create state file** using WorkflowStateManager.createState():
   ...
   - Writes state to `.sinapse/{instance-id}-state.yaml`
...
3. **Generate cleanup notes:**
   ...
   State file preserved at: .sinapse/{instance-id}-state.yaml
