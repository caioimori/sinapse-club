...
Complete environment bootstrap for new SINAPSE projects. Verifies and installs all required CLIs, authenticates services, initializes Git/GitHub repository, and validates the development environment before starting the greenfield workflow.
...
- campo: environment_report
  tipo: object
  destino: File system (.sinapse/environment-report.yaml)
  persistido: true
...
post-conditions:
  ...
  - [ ] Environment report generated
    ...
    blocker: false
    validação: |
      Check .sinapse/environment-report.yaml exists
...
acceptance-criteria:
  ...
  - [ ] Project structure follows SINAPSE conventions
    tipo: acceptance-criterion
    blocker: false
    validação: |
      Check docs/, .sinapse/, and package.json exist
...
- Cache detection results in .sinapse/environment-report.yaml
...
ESSENTIAL (required for SINAPSE):
...
# SINAPSE generated files
.sinapse/project-status.yaml
.sinapse/environment-report.yaml
...
---
*Generated by SINAPSE Environment Bootstrap*
...
- Initialize SINAPSE project structure
...
**Action:** Create SINAPSE-compliant project structure
...
# Create .sinapse/config.yaml
@"
# SINAPSE Project Configuration
version: 2.1.0
project:
  name: $PROJECT_NAME
  type: greenfield
...
"@ | Out-File -FilePath ".sinapse/config.yaml" -Encoding utf8
...
@"
{
  "name": "$PROJECT_NAME",
  "version": "0.1.0",
  "description": "Created with SINAPSE",
  "scripts": {
...
**Action:** Ask user for their profile preference and persist to `~/.sinapse/user-config.yaml`
...
```bash
# Create ~/.sinapse/ with secure permissions
mkdir -p ~/.sinapse
chmod 700 ~/.sinapse

# Write user-config.yaml with selected profile
cat > ~/.sinapse/user-config.yaml << EOF
# SINAPSE User Preferences (global, cross-project)
...
```javascript
const { setUserConfigValue, ensureUserConfigDir } = require('.sinapse-ai/core/config/config-resolver');
...
- `~/.sinapse/` directory must have permissions 700
- `~/.sinapse/user-config.yaml` must be valid YAML after write
...
$report | ConvertTo-Json -Depth 5 | Out-File -FilePath ".sinapse/environment-report.json" -Encoding utf8

Write-Host "✅ Environment report saved to .sinapse/environment-report.json"
```
...
```
╔═══════════════════════════════════════════════════════════════════════════╗
║              ✅ SINAPSE ENVIRONMENT BOOTSTRAP COMPLETE                        ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
...
║                                                                            ║
║  Full workflow: .sinapse-ai/development/workflows/greenfield-fullstack.yaml ║
║                                                                            ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  Quick Reference                                                           ║
╠═══════════════════════════════════════════════════════════════════════════╣
...
- [SINAPSE Greenfield Workflow](.sinapse-ai/development/workflows/greenfield-fullstack.yaml)
