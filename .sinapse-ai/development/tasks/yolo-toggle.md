...
Read the current permission mode from `.sinapse/config.yaml` under `permissions.mode`.
...
Update `.sinapse/config.yaml` with the new `permissions.mode` value.
The `PermissionMode._saveToConfig()` method handles this, including creating the `.sinapse/` directory if needed.
...
```javascript
const { cycleMode } = require('./.sinapse-ai/core/permissions');
...
- If `.sinapse/config.yaml` cannot be read, start from default `ask` mode
