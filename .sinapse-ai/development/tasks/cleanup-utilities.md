...
const configPath = path.join(__dirname, '../../.sinapse-ai/core-config.yaml');
...
mkdir -p .backups
cp -r .sinapse-ai/utils ".backups/utils.backup-3.18-$(date +%Y%m%d-%H%M%S)"
```
...
grep -r "require.*utility-name" .sinapse-ai/agents .sinapse-ai/tasks .sinapse-ai/workflows Squads/

# Check for string references (utility mentioned in docs/configs)
grep -r "utility-name" .sinapse-ai/agents/ .sinapse-ai/tasks/ .sinapse-ai/core-config.yaml

# Count total references
count=$(grep -r "utility-name" .sinapse-ai/ Squads/ 2>/dev/null | wc -l)
...
```bash
mkdir -p .sinapse-ai/utils-archive
```

**3.2 Create Archive README**

Create `.sinapse-ai/utils-archive/ARCHIVE-README.md`:
...
1. **Copy file back**:
   ```bash
   cp .sinapse-ai/utils-archive/utility-name.js .sinapse-ai/scripts/
   ```
...
4. **Test thoroughly**:
   ```bash
   node .sinapse-ai/scripts/test-utilities.js
   ```
...
**Immediate Rollback**:
```bash
rm -rf .sinapse-ai/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .sinapse-ai/utils
```

**Selective Restoration**:
```bash
cp .sinapse-ai/utils-archive/specific-utility.js .sinapse-ai/scripts/
```
...
git mv .sinapse-ai/scripts/utility-name.js .sinapse-ai/utils-archive/

# Or for multiple files:
git mv .sinapse-ai/scripts/{sinapse-validator-fixed.js,sinapse-validator-refactored.js} .sinapse-ai/utils-archive/
```
...
git mv .sinapse-ai/scripts/sinapse-validator.test.js tests/utils/
```
...
1. sinapse-validator-fixed.js - Duplicate of sinapse-validator.js
2. sinapse-validator-refactored.js - Duplicate of sinapse-validator.js
...
...
Update utility count in `.sinapse-ai/core-config.yaml`:
...
```bash
node .sinapse-ai/scripts/sinapse-validator.js
```
...
for util in $(ls .sinapse-ai/utils-archive/*.js); do
  name=$(basename $util .js)
  refs=$(grep -r "require.*$name" .sinapse-ai/agents .sinapse-ai/tasks 2>/dev/null | wc -l)
  if [ $refs -gt 0 ]; then
...
rm -rf .sinapse-ai/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .sinapse-ai/utils
git checkout .sinapse-ai/core-config.yaml
```
...
1. `.sinapse-ai/utils-archive/` - Archive directory with 30 deprecated utilities
2. `.sinapse-ai/utils-archive/ARCHIVE-README.md` - Archive documentation
3. `.backups/utils.backup-3.18-YYYYMMDD-HHMMSS/` - Timestamped backup
4. Updated `.sinapse-ai/core-config.yaml` - Corrected utility count
