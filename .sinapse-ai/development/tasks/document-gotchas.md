...
- campo: gotchas_file
  tipo: string
  destino: .sinapse/gotchas.md
  persistido: true

- campo: gotchas_json
  tipo: string
  destino: .sinapse/gotchas.json
  persistido: true
...
```yaml
pre-conditions:
  - [ ] Project has .sinapse directory initialized
    tipo: pre-condition
    blocker: false
    validação: |
      Check if .sinapse/ directory exists, create if not
    error_message: "Creating .sinapse/ directory"
...
post-conditions:
  - [ ] gotchas.md file generated/updated
    ...
    blocker: true
    validação: |
      Check .sinapse/gotchas.md exists and has content
  ...
  - [ ] gotchas.json schema valid
    ...
    blocker: false
    validação: |
      Validate .sinapse/gotchas.json against schema
...
1. **update** (default)
   ...
   - Generates `.sinapse/gotchas.md` and `.sinapse/gotchas.json`
...
```javascript
const { GotchasDocumenter } = require('.sinapse-ai/infrastructure/scripts/gotchas-documenter');

const documenter = new GotchasDocumenter(rootPath, {
  outputPath: '.sinapse/gotchas.md',
  quiet: false
});
```
...
documenter.mergeWithExisting('.sinapse/gotchas.json');
```
...
afterCapture: async (insightsPath) => {
  const { updateGotchas } = require('.sinapse-ai/infrastructure/scripts/gotchas-documenter');
...
const { getGotchasForSelfCritique } = require('.sinapse-ai/infrastructure/scripts/gotchas-documenter');
...
node .sinapse-ai/infrastructure/scripts/gotchas-documenter.js update

# List all gotchas
node .sinapse-ai/infrastructure/scripts/gotchas-documenter.js list

# List high severity only
node .sinapse-ai/infrastructure/scripts/gotchas-documenter.js list --severity high

# Search for specific gotchas
node .sinapse-ai/infrastructure/scripts/gotchas-documenter.js search "zustand"

# List by category
node .sinapse-ai/infrastructure/scripts/gotchas-documenter.js category TypeScript

# Output as JSON
node .sinapse-ai/infrastructure/scripts/gotchas-documenter.js list --format json
```
...
3. **Error:** Write permission denied
   - **Cause:** Cannot write to .sinapse directory
...
**Script:** gotchas-documenter.js
  ...
  - **Language:** JavaScript
  - **Location:** .sinapse-ai/infrastructure/scripts/gotchas-documenter.js

---

## Dependencies

- `.sinapse-ai/development/tasks/capture-session-insights.md` - Provides input insights
- `.sinapse-ai/product/checklists/self-critique-checklist.md` - Consumes gotchas (AC5)
- `.sinapse-ai/development/tasks/spec-write-spec.md` - May reference gotchas
