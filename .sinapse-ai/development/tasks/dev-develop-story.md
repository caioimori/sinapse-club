...
} = require('./.sinapse-ai/scripts/decision-recorder');
```
...
2. **Code Intelligence Check (IDS G4)** — Before creating new files or functions:
   - If code intelligence is available (`isCodeIntelAvailable()` from `.sinapse-ai/core/code-intel`):
     - Call `checkBeforeWriting(fileName, description)` from `.sinapse-ai/core/code-intel/helpers/dev-helper`
...
7. Execute `.sinapse-ai/product/checklists/story-dod-checklist.md`
...
│    │    wsl bash -c 'cd /mnt/c/.../sinapse-ai &&    │   │
...
- `.sinapse-ai/product/checklists/story-dod-checklist.md` - Definition of Done checklist
