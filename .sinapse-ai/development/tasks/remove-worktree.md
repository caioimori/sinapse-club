...
post-conditions:
  - [ ] Worktree directory removed
    tipo: post-condition
    blocker: true
    validação: Directory .sinapse/worktrees/{storyId} does not exist
...
Removes an SINAPSE-managed worktree and its associated branch. Includes safety checks for uncommitted changes and provides options for force removal.
...
Options:
  1. Commit changes first    : cd .sinapse/worktrees/{storyId} && git commit
...
You are about to remove:
  • Worktree: .sinapse/worktrees/{storyId}
  • Branch:   auto-claude/{storyId}
...
1. `git worktree remove .sinapse/worktrees/{storyId}`
...
❌ Cannot remove worktree while inside it.
   ...
   Current directory: .sinapse/worktrees/{storyId}
