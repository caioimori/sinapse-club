...
- campo: epic_state
  tipo: object
  destino: File system (.sinapse/epic-{epicId}-state.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] Template workflow must exist
    ...
    blocker: true
    validação: |
      execution.template must resolve to .sinapse-ai/development/workflows/{template}.yaml
  ...
  - [ ] For action=continue, state file must exist
    ...
    blocker: true
    validação: |
      .sinapse/epic-{epicId}-state.yaml must exist with status=active
...
post-conditions:
  - [ ] State file updated with current wave progress
    ...
    blocker: true
    validação: |
      .sinapse/epic-{epicId}-state.yaml exists and reflects completed waves/stories
...
4. **Error:** State file corrupted
   ...
   - **Recovery:** Restore from .sinapse/epic-{epicId}-state.yaml.bak
...
- Template `epic-orchestration.yaml` exists in `.sinapse-ai/development/workflows/`
- Inner loop `development-cycle.yaml` exists in `.sinapse-ai/development/workflows/`
...
```yaml
# .sinapse/epic-{epicId}-state.yaml
epic_state:
  epicId: {epicId}
...
**1. Load state** from `.sinapse/epic-{epicId}-state.yaml`
...
PROCEDURE execute_wave(wave, stories, state):
  ...
  Task tool call:
    ...
    subagent_type: "sinapse-architect"  # or whatever the gate agent is
    prompt: |
...
PROCEDURE final_gate(execution, state):
  ...
  Task tool call:
    ...
    subagent_type: "sinapse-architect"
    prompt: |
