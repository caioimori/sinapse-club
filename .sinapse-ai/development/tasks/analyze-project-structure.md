...
**Purpose:** Analyze an existing SINAPSE project to understand its structure, services, patterns, and provide recommendations for implementing new features. This is Phase 1 of the Incremental Feature Workflow.
...
- field: project_path
  ...
  required: false
  validation: Valid directory path with .sinapse-ai/
...
```yaml
pre-conditions:
  - [ ] Project has .sinapse-ai/ directory
    type: pre-condition
    blocker: true
    validation: |
      Check .sinapse-ai/ directory exists in project root
    error_message: "Pre-condition failed: Not an SINAPSE project (.sinapse-ai/ not found)"
...
acceptance-criteria:
  - [ ] Project structure scanned
    ...
    blocker: true
    validation: |
      Assert .sinapse-ai/ configuration analyzed
...
1. **Error:** No .sinapse-ai/ Directory
   - **Cause:** Not an SINAPSE project
   - **Resolution:** Initialize SINAPSE first or check directory
...
```javascript
// Core SINAPSE structure
const scanLocations = {
  sinapseCore: '.sinapse-ai/',
  services: '.sinapse-ai/infrastructure/services/',
  squads: '.sinapse-ai/squads/',
  agents: '.sinapse-ai/development/agents/',
  tasks: '.sinapse-ai/development/tasks/',
  data: '.sinapse-ai/data/'
};
```
...
const configPatterns = {
  ...
  configFile: exists('sinapse.config.js') || exists('.sinapse-ai/core-config.yaml'),
...
```
.sinapse-ai/infrastructure/services/{service-name}/
...
```
.sinapse-ai/infrastructure/services/{service_name}/
├── README.md
├── index.ts
...
```javascript
const { isCodeIntelAvailable } = require('.sinapse-ai/core/code-intel');
const { getDependencyGraph, getComplexityAnalysis } = require('.sinapse-ai/core/code-intel/helpers/planning-helper');
