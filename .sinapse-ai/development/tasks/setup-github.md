...
Configure complete GitHub DevOps infrastructure for user projects created with SINAPSE. This task copies GitHub Actions workflows, configures CodeRabbit, sets up branch protection, and manages secrets.
...
- campo: devops_setup_report
  tipo: object
  destino: File system (.sinapse/devops-setup-report.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] Not already configured (idempotency check)
    ...
    blocker: false
    validação: |
      Check .sinapse/devops-setup-report.yaml existence
...
post-conditions:
  ...
  - [ ] DevOps setup report generated
    ...
    blocker: false
    validação: |
      Test-Path ".sinapse/devops-setup-report.yaml"
...
acceptance-criteria:
  ...
  - [ ] Setup report documents all configurations
    ...
    blocker: true
    validação: |
      .sinapse/devops-setup-report.yaml contains all setup details
...
# Check idempotency
if (Test-Path ".sinapse/devops-setup-report.yaml") {
...
$ciTemplate = Get-Content ".sinapse-ai/infrastructure/templates/github-workflows/ci.yml.template" -Raw
...
Copy-Item ".sinapse-ai/infrastructure/templates/github-workflows/pr-automation.yml.template" `
  -Destination ".github/workflows/pr-automation.yml"
Write-Host "✅ Installed pr-automation.yml"

# Copy release.yml
Copy-Item ".sinapse-ai/infrastructure/templates/github-workflows/release.yml.template" `
...
$coderabbitConfig = Get-Content ".sinapse-ai/infrastructure/templates/coderabbit.yaml.template" -Raw
...
# Ensure .sinapse directory exists
New-Item -ItemType Directory -Path ".sinapse" -Force | Out-Null

$report | Out-File -FilePath ".sinapse/devops-setup-report.yaml" -Encoding utf8

Write-Host "✅ Setup report saved to .sinapse/devops-setup-report.yaml"
```
...
║     git add .github/ .coderabbit.yaml .sinapse/                              ║
║     git commit -m "chore: add DevOps configuration [Story 5.10]"          ║
║     git push                                                               ║
║                                                                            ║
║  Report saved: .sinapse/devops-setup-report.yaml                             ║
║                                                                            ║
╚═══════════════════════════════════════════════════════════════════════════╝
