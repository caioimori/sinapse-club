...
pre-conditions:
  ...
  - [ ] Brownfield Analyzer module is available
    ...
    blocker: true
    validation: |
      Verify .sinapse-ai/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js exists
...
- **Tool:** brownfield-analyzer
  ...
  - **Source:** .sinapse-ai/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js

- **Tool:** mode-detector
  - **Purpose:** Collect project markers for analysis
  - **Source:** .sinapse-ai/infrastructure/scripts/documentation-integrity/mode-detector.js
...
- **Script:** brownfield-analyzer.js
  ...
  - **Language:** JavaScript
  - **Location:** .sinapse-ai/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js
...
Analyze an existing project to understand its structure, tech stack, coding standards, and CI/CD workflows before SINAPSE integration. This task provides recommendations for safe integration and identifies potential conflicts.
...
```javascript
const { analyzeProject, formatMigrationReport } = require('./.sinapse-ai/infrastructure/scripts/documentation-integrity/brownfield-analyzer');
...
╠══════════════════════════════════════════════════════════════════════╣
║                                                                      ║
║  • Preserve existing ESLint configuration - SINAPSE will adapt          ║
║  • Keep existing Prettier settings - SINAPSE coding-standards.md will d ║
║  • Review existing CI/CD before adding SINAPSE workflows                ║
║  • SINAPSE will use existing tsconfig.json settings                     ║
...
|----------|---------|---------|
| `parallel` | Safe to proceed with standard SINAPSE setup | Use `*setup-project-docs` directly |
...
1. **Review GitHub Workflows:**
   ...
   # Check for potential conflicts with SINAPSE workflows
...
1. **docs/architecture/ exists:**
   - Decide: Keep existing or merge with SINAPSE docs
   - Option A: Rename existing to `docs/legacy-architecture/`
   - Option B: Configure SINAPSE to use alternate path
...
- Run this task BEFORE any SINAPSE integration
