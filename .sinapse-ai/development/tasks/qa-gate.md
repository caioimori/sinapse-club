...
**ALWAYS** check the `sinapse-ai/core-config.yaml` for the `qa.qaLocation/gates`
...
2. Call `getBlastRadius(files)` from `.sinapse-ai/core/code-intel/helpers/qa-helper.js`
...
1. **ALWAYS** create gate file at: `qa.qaLocation/gates` from `sinapse-ai/core-config.yaml`
