...
SINAPSE 4.0.4 runtime-first mode adds deterministic next-step recommendation from
...
pre-conditions:
  ...
  - [ ] Session state exists (optional)
    tipo: pre-condition
    blocker: false
    validação: Check .sinapse/session-state.json exists
...
```javascript
const SuggestionEngine = require('.sinapse-ai/workflow-intelligence/engine/suggestion-engine');
...
```javascript
const formatter = require('.sinapse-ai/workflow-intelligence/engine/output-formatter');
