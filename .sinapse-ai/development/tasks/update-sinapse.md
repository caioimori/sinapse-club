...
Git-native sync of SINAPSE framework from upstream repository. Uses sparse clone + file comparison for safe review before applying changes. All local customizations preserved automatically by backup/restore.
...
bash .sinapse-ai/scripts/update-sinapse.sh

# Review changes shown by the script, then:
git add .sinapse-ai && git commit -m "chore: sync SINAPSE framework"   # Apply changes
# OR
git checkout -- .sinapse-ai/                                         # Cancel changes
```
...
| Path | Reason |
|------|--------|
| `.sinapse-ai/squads/` | Custom copywriters, data, ralph |
| `.sinapse-ai/marketing/` | Marketing-specific agents/tasks |
| `source/` | Business context YAML |
| `Knowledge/` | Knowledge bases |
| `.sinapse-ai/context/` | Compiled contexts |
...
```yaml
task: updateSINAPSEFramework
agent: devops
mode: simple
timeout: 60  # 1 minute max

execution:
  script: .sinapse-ai/scripts/update-sinapse.sh

workflow:
  1. If dirty working tree: git add -A && git commit -m "chore: pre-update commit"
  2. bash .sinapse-ai/scripts/update-sinapse.sh
  3. Review changes displayed
  4. git add .sinapse-ai && git commit -m "chore: sync SINAPSE framework"  # to apply
  5. git checkout -- .sinapse-ai/                                        # to cancel
...
ls -la .sinapse-ai/squads/  # if exists
