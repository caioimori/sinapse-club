...
5. PUBLISH
   ...
   - Verify: npm view sinapse-ai@preview version === {new_version}

6. SMOKE TEST
   - Create temp directory
   - npm init -y
   - npm install sinapse-ai@preview
   - Verify critical files exist in node_modules/sinapse-ai/:
...
8. REPORT
   ...
   - "Test with: npm install sinapse-ai@preview"
...
- [ ] `npm view sinapse-ai@preview` returns new version
...
1. VERIFY PREVIEW EXISTS
   - npm view sinapse-ai@preview version
   - If no preview: HALT with "No preview version found"

2. CONFIRM WITH USER
   - Display: "Promote v{X.Y.Z} from preview to latest?"
   - Show current latest: npm view sinapse-ai@latest version
   - Require explicit confirmation

3. PROMOTE
   - npm dist-tag add sinapse-ai@{version} latest

4. VERIFY
   - npm view sinapse-ai@latest version === {version}
...
6. REPORT
   ...
   - Release URL
   - "Install with: npm install sinapse-ai"
```

#### Pre-conditions

- [ ] Preview version exists (`npm view sinapse-ai@preview`)
...
- [ ] `npm view sinapse-ai@latest` returns promoted version
...
2. INSTALL
   - npm install sinapse-ai@{tag} (default: latest)
   - Record: install time, exit code, warnings

3. VERIFY FILES
   - Check node_modules/sinapse-ai/ contains:
     - .sinapse-ai/core-config.yaml
     - .sinapse-ai/constitution.md
     - .sinapse-ai/development/agents/ (non-empty)
     - .sinapse-ai/development/tasks/ (non-empty)
     - .claude/hooks/synapse-engine.cjs
     - .claude/hooks/precompact-session-digest.cjs
     - .claude/rules/ (non-empty)
     - bin/sinapse.js
     - bin/sinapse-minimal.js
   - Check node_modules/sinapse-ai/ does NOT contain:
     - pro/
     - .env
     - .git/
     - tests/
...
4. TEST INSTALLER (optional, if --full flag)
   - npx sinapse-ai install --preset minimal
   - Verify: .sinapse-ai/ created
...
- `--full`: Also test `npx sinapse-ai install`
...
npm view sinapse-ai versions --json

# 2. Point latest back to previous version
npm dist-tag add sinapse-ai@{previous-good-version} latest

# 3. Deprecate broken version with message
npm deprecate sinapse-ai@{broken-version} "Known issues, use v{previous-good-version}"
...
To fully block a version, use `npm unpublish sinapse-ai@{version}` (within 72h of publish only).
