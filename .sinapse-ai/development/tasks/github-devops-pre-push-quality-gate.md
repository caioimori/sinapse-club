...
- Repository context detected (run `sinapse init` if needed)
...
if (!context) {
  ...
  console.error('Run "sinapse init" to configure installation mode');
  process.exit(1);
}
...
```javascript
const { assessPrePushImpact, classifyRiskLevel } = require('.sinapse-ai/core/code-intel/helpers/devops-helper');
