...
autoClaude:
  ...
  inputs:
    ...
    - name: projectContext
      type: file
      path: .sinapse/project-context.yaml
      required: false
...
step_11:
  ...
  actions:
    ...
    - action: update_dashboard_status
      script: |
        const { PlanTracker } = require('.sinapse-ai/infrastructure/scripts/plan-tracker.js');
...
recovery:
  ...
  attempt_tracking:
    storage: .sinapse/recovery/{storyId}/{subtaskId}.json
    schema:
      subtaskId: string
      attempts: array
