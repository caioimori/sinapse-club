...
outputs:
  - name: service_directory
    type: directory
    location: ".sinapse-ai/infrastructure/services/{service_name}/"
    persistido: true
...
```yaml
pre-conditions:
  - [ ] WIS-10 templates exist at .sinapse-ai/development/templates/service-template/
    tipo: pre-condition
    blocker: true
  ...
  - [ ] Service name is unique (no existing service with same name)
    tipo: pre-condition
    blocker: true
    validação: Check .sinapse-ai/infrastructure/services/{name}/ does not exist
...
3. agent-tool - Tool for SINAPSE agents
...
const { checkBeforeWriting } = require('.sinapse-ai/core/code-intel/helpers/dev-helper');
...
// Check uniqueness
const targetDir = `.sinapse-ai/infrastructure/services/${serviceName}/`;
...
```javascript
const templateDir = '.sinapse-ai/development/templates/service-template/';
const templates = [
  'README.md.hbs',
  'index.ts.hbs',
  'types.ts.hbs',
...
cd .sinapse-ai/infrastructure/services/{service_name}/
...
============================================
 ...
 Next Steps:
   1. cd .sinapse-ai/infrastructure/services/{service_name}
