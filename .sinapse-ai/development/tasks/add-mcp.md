...
### 7. Update SINAPSE Documentation (REQUIRED)
...
**CRITICAL NOTE:** Tools added to Docker MCP Toolkit are NOT immediately available to SINAPSE agents. The docker-gateway caches tools at Claude Code startup. User MUST restart their Claude Code session for new tools to appear.
...
🔗 Status: Running
📋 Preset: Added to 'sinapse-full'

Next steps:
1. Test tools: docker mcp tools call {name}.tool1 --param value
2. Use in workflow: *mcp-workflow with {name} tools
3. Add to other presets: docker mcp preset update sinapse-dev --add-server {name}
```
...
changelog:
  ...
  1.2.0:
    - Added: Steps 7-9 for SINAPSE documentation and session restart
    ...
    - Fixed: Tools not appearing in SINAPSE agents after MCP addition
  1.1.0:
