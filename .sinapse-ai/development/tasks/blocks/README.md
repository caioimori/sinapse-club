...
Blocks are small, focused units of functionality that can be included in multiple tasks. They follow the DRY (Don't Repeat Yourself) principle and provide consistent behavior across the SINAPSE framework.

## Directory Structure

```
.sinapse-ai/development/tasks/blocks/
├── README.md                  # This file
...
```javascript
const { loadBlock, executeBlock } = require('.sinapse-ai/utils/block-loader');
...
| `agent-prompt-template` | Standardized template for spawning SINAPSE agents | Any skill/task that invokes agents via Task tool |
