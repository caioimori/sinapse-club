...
Load SINAPSE project context before task execution. Provides git state, gotchas filtered by category, technical preferences, and core configuration.
...
steps:
  ...
  - name: Load Technical Preferences
    ...
    actions:
      - read: .sinapse-ai/data/technical-preferences.md
    output: preferences

  - name: Load Core Config
    actions:
      - read: .sinapse-ai/core-config.yaml
...
```javascript
const { loadContext } = require('.sinapse-ai/utils/context-loader');
...
| File | Purpose |
|------|---------|
| `.sinapse/gotchas.json` | Known issues and workarounds |
| `.sinapse-ai/data/technical-preferences.md` | User-defined patterns |
| `.sinapse-ai/core-config.yaml` | Project configuration |
