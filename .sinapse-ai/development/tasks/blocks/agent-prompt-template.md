...
Standardized template for spawning SINAPSE agents with consistent structure. Ensures all agent invocations follow the same persona loading, context, mission, and output pattern.
...
```markdown
You are {agent_name}, the SINAPSE {agent_role}. Read your complete agent file at:
{agent_file_path}
...
<!-- Parameters:
  ...
  agent_file_path=.claude/commands/SINAPSE/agents/architect.md,
...
```javascript
const { loadBlock, renderTemplate } = require('.sinapse-ai/utils/block-loader');
