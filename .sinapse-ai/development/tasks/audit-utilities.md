...
Systematically audit all utilities in `.sinapse-ai/scripts/` to determine their functional status, classify them as WORKING/FIXABLE/DEPRECATED, and generate actionable recommendations for maintenance and cleanup.
...
for util in .sinapse-ai/scripts/*.js; do
  name=$(basename $util .js)
  count=$(grep -r "$name" .sinapse-ai/agents .sinapse-ai/tasks Squads 2>/dev/null | wc -l)
