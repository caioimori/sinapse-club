...
- PM tool configured in `.sinapse-pm-config.yaml` (or will use local-only mode)
...
```javascript
const { getPMAdapter } = require('../.sinapse-ai/scripts/pm-adapter-factory');
...
- **Configuration missing**: Inform user to run `sinapse init`
...
```javascript
const { syncStoryToPM } = require('../.sinapse-ai/scripts/story-manager');
