...
outputs:
  - name: patterns_markdown
    type: string
    destino: File (.sinapse/patterns.md)
    persistido: true
...
pre-conditions:
  - [ ] Project root is a valid codebase
    tipo: pre-condition
    blocker: true
    validação: Check package.json or .sinapse-ai exists
...
```javascript
const PatternExtractor = require('.sinapse-ai/infrastructure/scripts/pattern-extractor');
...
Subcommands:
  ...
  save        Save to .sinapse/patterns.md
...
Examples:
  ...
  *extract-patterns save                      # Save to .sinapse/patterns.md
...
Output File:
  Default location: .sinapse/patterns.md
...
Patterns saved to: .sinapse/patterns.md
...
post-conditions:
  - [ ] Patterns file created/updated
    tipo: post-condition
    blocker: false
    validação: Check .sinapse/patterns.md exists and is valid markdown
...
node .sinapse-ai/infrastructure/scripts/pattern-extractor.js [command] [options]
