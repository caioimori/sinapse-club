...
# Suggest Refactoring - SINAPSE Developer Task
...
```bash
# Analyze single file
*suggest-refactoring sinapse-ai/scripts/complex-utility.js

# Analyze directory with specific patterns
*suggest-refactoring sinapse-ai/agents --patterns extract_method,decompose_class --recursive

# Apply high-impact suggestions
*suggest-refactoring sinapse-ai/utils --threshold 7 --apply-all

# Export suggestions for review
*suggest-refactoring . --recursive --export refactoring-report.json

# Dry run to see changes
*suggest-refactoring sinapse-ai/agents/developer.md --apply ref-001 --dry-run
```
...
class SuggestRefactoringTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('🔧 SINAPSE Refactoring Analysis'));
  ...
  async loadSuggestion(suggestionId) {
    ...
    const cacheFile = path.join(this.rootPath, '.sinapse', 'refactoring', `${suggestionId}.json`);
...
6. **Code Intelligence Blast Radius** (if available):
   - Call `assessRefactoringImpact(candidateFiles)` from `.sinapse-ai/core/code-intel/helpers/dev-helper`
