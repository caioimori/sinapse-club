...
```yaml
pre-conditions:
  - [ ] Entity Registry exists at .sinapse-ai/data/entity-registry.yaml
    tipo: pre-condition
    blocker: false
    validação: |
...
```bash
# Basic query
node bin/sinapse-ids.js ids:query "validate story drafts"

# With JSON output
node bin/sinapse-ids.js ids:query "template rendering engine" --json

# Filter by type
node bin/sinapse-ids.js ids:query "database migration" --type script

# Filter by category
node bin/sinapse-ids.js ids:query "agent persona" --category agents
```
...
const { RegistryLoader } = require(path.resolve('.sinapse-ai/core/ids/registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve('.sinapse-ai/core/ids/incremental-decision-engine'));
...
- `sinapse ids:create-review` — Review CREATE decisions for 30-day assessment
