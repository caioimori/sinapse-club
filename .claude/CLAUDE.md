...
O SINAPSE carrega regras contextuais de `.claude/rules/` automaticamente. Regras com frontmatter `paths:` só carregam quando arquivos correspondentes são editados.
...
O SINAPSE possui um sistema de code intelligence opcional que enriquece operações com dados de análise de código.
...
```bash
sinapse graph --deps                        # Dependency tree (ASCII)
sinapse graph --deps --format=json          # Output como JSON
sinapse graph --deps --format=html          # Interactive HTML (abre browser)
sinapse graph --deps --format=mermaid       # Mermaid diagram
sinapse graph --deps --format=dot           # DOT format (Graphviz)
...
---
*SINAPSE Claude Code Configuration v2.0*
