 ...
 * the unified hook runner in sinapse-ai.
 *
 ...
 * }
 *
 * @see .sinapse-ai/hooks/unified/runners/precompact-runner.js
 ...
 * Framework-dev: PROJECT_ROOT/.sinapse-ai/hooks/unified/runners/precompact-runner.js
 * Installed:     PROJECT_ROOT/node_modules/sinapse-ai/.sinapse-ai/hooks/unified/runners/precompact-runner.js
 */
function resolveRunnerPath() {
  const fs = require('fs');
  const candidates = [
    path.join(PROJECT_ROOT, '.sinapse-ai', 'hooks', 'unified', 'runners', 'precompact-runner.js'),
    path.join(PROJECT_ROOT, 'node_modules', 'sinapse-ai', '.sinapse-ai', 'hooks', 'unified', 'runners', 'precompact-runner.js'),
  ];
...
async function main() {
  ...
  // The child receives context via SINAPSE_HOOK_CONTEXT env var and calls
  ...
  try {
    ...
    const child = spawn(process.execPath, ['-e', inlineScript], {
      detached: true,
      stdio: 'ignore',
      env: { ...process.env, SINAPSE_HOOK_CONTEXT: contextJson },
    });
    child.on('error', () => {});
    child.unref();
  } catch (_) {
