...
wsl bash -c 'cd /mnt/c/.../sinapse-ai && ~/.local/bin/coderabbit --severity CRITICAL,HIGH --auto-fix'

# Manual review
wsl bash -c 'cd /mnt/c/.../sinapse-ai && ~/.local/bin/coderabbit -t uncommitted'

# Prompt-only mode
wsl bash -c 'cd /mnt/c/.../sinapse-ai && ~/.local/bin/coderabbit --prompt-only -t uncommitted'
```
...
Full config in `.sinapse-ai/core-config.yaml` under `coderabbit_integration` section.
